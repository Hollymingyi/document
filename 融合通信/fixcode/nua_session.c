...
char* getReqSdpPt(char* sdp) {
	...
	while (token != NULL)
	{
		char *level = strstr(token, "profile-level-id=");
		if (level != NULL) {
			char *fmtp = strstr(token, "a=fmtp:");
			strncpy(result, fmtp + 7, 4);
			return result;
		}
		token = strtok(NULL, "\n");
	}
...
void changeSdp(char* resp_sdp,int old_pt,int new_pt){
	char old_proto[20];
	sprintf(old_proto, "RTP/AVP %d", old_pt);
	char new_proto[20];
	sprintf(new_proto, "RTP/AVP %d", new_pt);

	char old_rm[20];
	sprintf(old_rm, "a=rtpmap:%d", old_pt);
	char new_rm[20];
	sprintf(new_rm, "a=rtpmap:%d", new_pt);

	char old_fmtp[20];
	sprintf(old_fmtp, "a=fmtp:%d", old_pt);
	char new_fmtp[20];
	sprintf(new_fmtp, "a=fmtp:%d", new_pt);

	char old_fb[20];
	sprintf(old_fb, "a=rtcp-fb:%d", old_pt);
	char new_fb[20];
	sprintf(new_fb, "a=rtcp-fb:%d", new_pt);
	replaceAll(resp_sdp, old_proto, new_proto);
	replaceAll(resp_sdp, old_rm, new_rm);
	replaceAll(resp_sdp, old_fmtp, new_fmtp);
	replaceAll(resp_sdp, old_fb, new_fb);
}
...
int nua_update_server_respond(nua_server_request_t *sr, tagi_t const *tags)
{
  ...
  if (sr->sr_response.sip && sr->sr_sdp && sr->sr_response.sip->sip_payload->pl_data) {
	  char *req_level = getReqSdpLevel(sr->sr_sdp);
	  char *req_pt = getReqSdpPt(sr->sr_sdp);
	  //SU_DEBUG_5(("sdp : %s \n", sr->sr_sdp));
	  char *resp_level = getReqSdpLevel(sr->sr_response.sip->sip_payload->pl_data);
	  char *resp_pt = getReqSdpPt(sr->sr_response.sip->sip_payload->pl_data);
	  SU_DEBUG_5(("nua(%p): %s server: error activating media\n",
		  (void *)nh, "UPDATE"));
	  if (req_pt != NULL && resp_pt != NULL) {
		if(atoi(req_pt) != atoi(resp_pt)){
			//replaceAll(sr->sr_response.sip->sip_payload->pl_data,resp_level , req_level);
			changeSdp(sr->sr_response.sip->sip_payload->pl_data, atoi(resp_pt), atoi(req_pt));
			//SU_DEBUG_9(("new sdp = %s \n", sr->sr_response.sip->sip_payload->pl_data));
		}
	  }
	  free(req_level);
	  free(req_pt);
