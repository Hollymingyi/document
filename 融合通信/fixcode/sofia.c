...
static void parse_domain_tag(sofia_profile_t *profile, switch_xml_t x_domain_tag, const char *dname, const char *parse,
							 const char *alias);

void sofia_handle_sip_i_reinvite(switch_core_session_t *session, nua_t *nua, sofia_profile_t *profile, nua_handle_t *nh,
								 sofia_private_t *sofia_private, sip_t const *sip, sofia_dispatch_event_t *de,
								 tagi_t tags[]);

static void set_variable_sip_param(switch_channel_t *channel, char *header_type, sip_param_t const *params);

static void sofia_handle_sip_i_state(switch_core_session_t *session, int status, char const *phrase, nua_t *nua,
									 sofia_profile_t *profile, nua_handle_t *nh, sofia_private_t *sofia_private,
									 sip_t const *sip, sofia_dispatch_event_t *de, tagi_t tags[]);

static void sofia_handle_sip_r_invite(switch_core_session_t *session, int status, char const *phrase, nua_t *nua,
									  sofia_profile_t *profile, nua_handle_t *nh, sofia_private_t *sofia_private,
									  sip_t const *sip, sofia_dispatch_event_t *de, tagi_t tags[]);
static void sofia_handle_sip_r_options(switch_core_session_t *session, int status, char const *phrase, nua_t *nua,
									   sofia_profile_t *profile, nua_handle_t *nh, sofia_private_t *sofia_private,
									   sip_t const *sip, sofia_dispatch_event_t *de, tagi_t tags[]);
...
static void sofia_set_accept_language_channel_variable(switch_channel_t *channel, sip_t const *sip)
{
	if (sip->sip_accept_language) {
		...
		for (sip_accept_language = sip->sip_accept_language; sip_accept_language;
			 sip_accept_language = sip_accept_language->aa_next) {
			char var_name[64] = "";

			if (zstr(sip_accept_language->aa_value)) { continue; }
			if (count == 0) {
...
void sofia_handle_sip_r_notify(switch_core_session_t *session, int status, char const *phrase, nua_t *nua,
							   sofia_profile_t *profile, nua_handle_t *nh, sofia_private_t *sofia_private,
							   sip_t const *sip, sofia_dispatch_event_t *de, tagi_t tags[])
{
...
#define url_set_chanvars(session, url, varprefix)                                                                      \
	_url_set_chanvars(session, url, #varprefix "_user", #varprefix "_host", #varprefix "_port", #varprefix "_uri",     \
					  #varprefix "_params")
									 ...
									 const char *params_var)
{
	...
	if (url) {
		...
		host = url->url_host;
		port = url->url_port;
		if (!zstr(url->url_params)) { switch_channel_set_variable(channel, params_var, url->url_params); }
	}

	if (zstr(user)) { user = "nobody"; }

	if (zstr(host)) { host = "nowhere"; }

	check_decode(user, session);

	if (user) { switch_channel_set_variable(channel, user_var, user); }

	if (port) { switch_snprintf(new_port, sizeof(new_port), ":%s", port); }
								...
								nua_handle_t *nh)
{
	...
	if (sip) {
		...
		if (sip->sip_record_route) {
			...
			if (switch_channel_direction(channel) == SWITCH_CALL_DIRECTION_OUTBOUND ||
				switch_channel_test_flag(channel, CF_RECOVERED)) {
		...
		if (sip->sip_via) {
			...
			if (switch_channel_direction(channel) == SWITCH_CALL_DIRECTION_OUTBOUND ||
				switch_stristr("TCP", (char *)stream.data)) {
...
static void extract_vars(sofia_profile_t *profile, sip_t const *sip, switch_core_session_t *session)
{
	...
	if (sip) {
		if (sip->sip_from) url_set_chanvars(session, sip->sip_from->a_url, sip_from);
		if (sip->sip_request) url_set_chanvars(session, sip->sip_request->rq_url, sip_req);
		if (sip->sip_to) url_set_chanvars(session, sip->sip_to->a_url, sip_to);
		if (sip->sip_contact) url_set_chanvars(session, sip->sip_contact->m_url, sip_contact);
		if (sip->sip_referred_by) url_set_chanvars(session, sip->sip_referred_by->b_url, sip_referred_by);
 ...
 */
static void sofia_add_invite_header_to_chanvars(switch_channel_t *channel, nua_handle_t *nh, void *sip_header,
												const char *var)
{
 ...
 * Finally sets the "sip_invite_headers" to a string containing the 'original' SIP headers, except that the order may
 * have changed. Multiple headers will have the original internal order, though.
 *
...
static void sofia_parse_all_invite_headers(sip_t const *sip, switch_core_session_t *session, nua_handle_t *nh)
{
	...
	if (sip->sip_proxy_authorization) {
		...
		for (vp = sip->sip_proxy_authorization; vp; vp = vp->au_next) {
			char *v = sip_header_as_string(nua_handle_get_home(nh), (void *)vp);
			switch_channel_add_variable_var_check(channel, "sip_i_proxy_authorization", v, SWITCH_FALSE,
												  SWITCH_STACK_PUSH);
			su_free(nua_handle_get_home(nh), v);
		}
	}
	...
	if ((passerted = sip_p_asserted_identity(sip))) {
		...
		for (vp = passerted; vp; vp = vp->paid_next) {
			char *v = sip_header_as_string(nua_handle_get_home(nh), (void *)vp);
			switch_channel_add_variable_var_check(channel, "sip_i_p_asserted_identity", v, SWITCH_FALSE,
												  SWITCH_STACK_PUSH);
			su_free(nua_handle_get_home(nh), v);
		}
	}
	...
	if ((ppreferred = sip_p_preferred_identity(sip))) {
		...
		for (vp = ppreferred; vp; vp = vp->ppid_next) {
			char *v = sip_header_as_string(nua_handle_get_home(nh), (void *)vp);
			switch_channel_add_variable_var_check(channel, "sip_i_p_preferred_identity", v, SWITCH_FALSE,
												  SWITCH_STACK_PUSH);
			su_free(nua_handle_get_home(nh), v);
		}
	}
	...
	for (un = sip->sip_unknown; un; un = un->un_next) {
		if (!zstr(un->un_name) && !zstr(un->un_value)) {
			...
			if ((parsed_name = switch_mprintf("sip_i_%s", un->un_name))) {
				...
				switch_channel_add_variable_var_check(channel, parsed_name, un->un_value, SWITCH_FALSE,
													  SWITCH_STACK_PUSH);
				free(parsed_name);
			}
		}
...
void sofia_handle_sip_i_notify(switch_core_session_t *session, int status, char const *phrase, nua_t *nua,
							   sofia_profile_t *profile, nua_handle_t *nh, sofia_private_t *sofia_private,
							   sip_t const *sip, sofia_dispatch_event_t *de, tagi_t tags[])
{
	...
	if (!sip || !sip->sip_event) { goto error; }
	...
	if (tech_pvt && tech_pvt->proxy_refer_uuid && sofia_test_pflag(profile, PFLAG_PROXY_REFER) && sip->sip_payload &&
		sip->sip_payload->pl_data && sip->sip_content_type && sip->sip_content_type->c_type &&
		switch_stristr("sipfrag", sip->sip_content_type->c_type)) {

		if (sofia_pass_notify(session, tech_pvt->proxy_refer_uuid, sip->sip_payload->pl_data) ==
	...
	if (sip->sip_content_type && sip->sip_content_type->c_type && sip->sip_payload && sip->sip_payload->pl_data &&
		...
		if (switch_event_create_subclass(&s_event, SWITCH_EVENT_CUSTOM, MY_EVENT_NOTIFY_REFER) ==
			SWITCH_STATUS_SUCCESS) {
	...
	if (s_event != NULL) {
		...
		if (sip->sip_contact) {
			switch_event_add_header(s_event, SWITCH_STACK_BOTTOM, "contact", "%s@%s", sip->sip_contact->m_url->url_user,
									sip->sip_contact->m_url->url_host);
		}

		switch_event_add_header(s_event, SWITCH_STACK_BOTTOM, "from", "%s@%s", sip->sip_from->a_url->url_user,
								sip->sip_from->a_url->url_host);
		switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "from-tag", sip->sip_from->a_tag);
		switch_event_add_header(s_event, SWITCH_STACK_BOTTOM, "to", "%s@%s", sip->sip_to->a_url->url_user,
		...
		if (sip->sip_subscription_state && sip->sip_subscription_state->ss_substate) {
			switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "subscription-substate",
										   sip->sip_subscription_state->ss_substate);
		}
		if (sip->sip_subscription_state && sip->sip_subscription_state->ss_reason) {
			switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "subscription-reason",
										   sip->sip_subscription_state->ss_reason);
		}
		if (sip->sip_subscription_state && sip->sip_subscription_state->ss_retry_after) {
			switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "subscription-retry-after",
										   sip->sip_subscription_state->ss_retry_after);
		}
		if (sip->sip_subscription_state && sip->sip_subscription_state->ss_expires) {
			switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "subscription-expires",
										   sip->sip_subscription_state->ss_expires);
		}
		if (session) {
			switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "UniqueID",
		...
		if (session && channel && tech_pvt) {
			if (sip->sip_payload && sip->sip_payload->pl_data) {
				...
				if (!status_val || status_val >= 200) {
					...
					if ((int)tech_pvt->want_event == 9999) { tech_pvt->want_event = 0; }
				} else if (status_val < 200) {
					switch_channel_set_variable_printf(channel, "sip_refer_target_provisional_status_code", "%d",
													   status_val);
				}
			}
		}
	...
	if (session) {
		if (!zstr(profile->proxy_notify_events) && (!strcasecmp(profile->proxy_notify_events, "all") ||
													strstr(profile->proxy_notify_events, sip->sip_event->o_type))) {
			switch_core_session_t *other_session;
			if (switch_core_session_get_partner(session, &other_session) == SWITCH_STATUS_SUCCESS) {
				...
				full_to = switch_str_nil(
					switch_channel_get_variable(switch_core_session_get_channel(other_session), "sip_full_to"));
				if (sip->sip_payload && sip->sip_payload->pl_data) {
		...
		if (strcasecmp(sip->sip_event->o_type, "talk")) { goto error; }

		if (switch_channel_direction(channel) == SWITCH_CALL_DIRECTION_INBOUND) {
			switch_channel_answer(channel);
			switch_channel_set_variable(channel, "auto_answer_destination",
										switch_channel_get_variable(channel, "destination_number"));
	...
	if (!sofia_private || zstr(sofia_private->gateway_name)) {
		if (profile->debug) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
							  "Gateway information missing Subscription Event: %s\n", sip->sip_event->o_type);
		}
		goto error;
	}
	...
	if (switch_event_create(&s_event, SWITCH_EVENT_NOTIFY_IN) == SWITCH_STATUS_SUCCESS) {
		...
		switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "pl_data",
									   sip->sip_payload ? sip->sip_payload->pl_data : "");
		if (sip->sip_content_type != NULL)
			switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "sip_content_type",
		...
		switch_event_fire(&s_event);
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
						  "dispatched freeswitch event for message-summary NOTIFY\n");
	} else {
...
error:
	...
	if (sip && sip->sip_event && sip->sip_event->o_type && !strcasecmp(sip->sip_event->o_type, "message-summary")) {
		...
		if (sofia_test_pflag(profile, PFLAG_FORWARD_MWI_NOTIFY)) {
			...
			if (sip->sip_to && sip->sip_to->a_url->url_user && sip->sip_to->a_url->url_host && sip->sip_payload &&
				...
				for (x = 0; x < profile->acl_count; x++) {
					...
					if (!(acl_ok = switch_check_network_list_ip(network_ip, last_acl))) { break; }
				}
				...
				if (acl_ok) {
					...
					if (mwi_status) {
						...
						if (switch_event_create(&s_event, SWITCH_EVENT_MESSAGE_WAITING) == SWITCH_STATUS_SUCCESS) {
							switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "MWI-Messages-Waiting",
														   mwi_stat);
							switch_event_add_header(s_event, SWITCH_STACK_BOTTOM, "MWI-Message-Account", "%s@%s",
													sip->sip_to->a_url->url_user, sip->sip_to->a_url->url_host);
							switch_event_fire(&s_event);
						}
					...
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
									  "Dropping unsolicited MWI ( %s@%s ) because of ACL\n",
									  sip->sip_to->a_url->url_user, sip->sip_to->a_url->url_host);
				};
			}
		}
	...
	if (!gateway && sub_state == nua_substate_terminated && sofia_private &&
		sofia_private != &mod_sofia_globals.destroy_private && sofia_private != &mod_sofia_globals.keep_private) {
		sofia_private->destroy_nh = 1;
		sofia_private->destroy_me = 1;
	}

	if (gateway) { sofia_reg_release_gateway(gateway); }
}

/*
 * This will fire an event containing X-headers from the BYE response
 */
void sofia_handle_sip_r_bye(switch_core_session_t *session, int status, char const *phrase, nua_t *nua,
							sofia_profile_t *profile, nua_handle_t *nh, sofia_private_t *sofia_private,
							sip_t const *sip, sofia_dispatch_event_t *de, tagi_t tags[])
{
	if (profile && sofia_test_pflag(profile, PFLAG_FIRE_BYE_RESPONSE_EVENTS) && sip && sip->sip_call_id &&
		!zstr(sip->sip_call_id->i_id) && sofia_private && !zstr_buf(sofia_private->uuid_str)) {
		switch_event_t *bye_response_event = NULL;
		sip_unknown_t *un;
		if (switch_event_create_subclass(&bye_response_event, SWITCH_EVENT_CUSTOM, MY_EVENT_BYE_RESPONSE) ==
			SWITCH_STATUS_SUCCESS) {
			switch_event_add_header(bye_response_event, SWITCH_STACK_BOTTOM, "call-id", "%s", sip->sip_call_id->i_id);
			switch_event_add_header(bye_response_event, SWITCH_STACK_BOTTOM, "Unique-ID", "%s",
									sofia_private->uuid_str);
			for (un = sip->sip_unknown; un; un = un->un_next) {
				if (!zstr(un->un_value)) {
...
void sofia_handle_sip_i_bye(switch_core_session_t *session, int status, char const *phrase, nua_t *nua,
							sofia_profile_t *profile, nua_handle_t *nh, sofia_private_t *sofia_private,
							sip_t const *sip, sofia_dispatch_event_t *de, tagi_t tags[])
{
	const char *tmp;
	...
	if (switch_channel_test_flag(tech_pvt->channel, CF_SLA_BARGE)) {
		...
		if (!zstr(cmd)) { switch_ivr_eavesdrop_exec_all(session, "transfer", cmd); }
	}
	...
	if (sip->sip_reason && sip->sip_reason->re_protocol &&
		(!strcasecmp(sip->sip_reason->re_protocol, "Q.850") ||
		 ...
		 !strcasecmp(sip->sip_reason->re_protocol, profile->sdp_username)) &&
...
void sofia_send_callee_id(switch_core_session_t *session, const char *name, const char *number)
{
	...
	if (switch_channel_inbound_display(channel)) {
		if (zstr(name)) { name = caller_profile->caller_id_name; }

		if (zstr(number)) { number = caller_profile->caller_id_number; }

		if (zstr(name)) { name = number; }

		if (zstr(number)) { name = number = "UNKNOWN"; }

		if (!zstr(name) && !strcmp(name, "_undef_")) { name = ""; }
	} else {
		if (zstr(name)) { name = caller_profile->callee_id_name; }

		if (zstr(number)) { number = caller_profile->callee_id_number; }

		if (zstr(name)) { name = number; }

		if (zstr(number)) { number = caller_profile->destination_number; }
							...
							switch_bool_t send)
{
	...
	if (switch_true(switch_channel_get_variable(channel, SWITCH_IGNORE_DISPLAY_UPDATES_VARIABLE)) ||
		!sofia_test_pflag(profile, PFLAG_SEND_DISPLAY_UPDATE)) {
		return;
	}
	...
	if (!fs) {
		sip_remote_party_id_t *rpid;
		if ((passerted = sip_p_asserted_identity(sip))) {
			if (passerted->paid_url->url_user) { number = passerted->paid_url->url_user; }
			if (!zstr(passerted->paid_display)) {
				...
				if (end_of(name) == '"') { end_of(name) = '\0'; }
			}
		} else if ((rpid = sip_remote_party_id(sip))) {
			if (rpid->rpid_url->url_user) { number = rpid->rpid_url->url_user; }
	...
	if (zstr(number)) {
		if ((tmp = switch_channel_get_variable(channel, num_var)) && !zstr(tmp)) { number = (char *)tmp; }

		if (zstr(number)) { number = "unknown"; }
	}

	if (zstr(name)) {
		if ((tmp = switch_channel_get_variable(channel, name_var)) && !zstr(tmp)) { name = (char *)tmp; }
	}

	if (zstr(name)) { name = (char *)number; }

	if (zstr(name) || zstr(number)) { goto end; }
	...
	if (switch_channel_inbound_display(channel)) {
		...
		if (!strcmp(caller_profile->callee_id_name, name) && !strcmp(caller_profile->callee_id_number, number)) {
			...
			if (lazy || (att && !switch_channel_get_partner_uuid(channel))) { switch_channel_flip_cid(channel); }
		}
	}
	...
	if (send) {
		...
		if (switch_event_create(&event, SWITCH_EVENT_CALL_UPDATE) == SWITCH_STATUS_SUCCESS) {
			...
			if (uuid) { switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Bridged-To", uuid); }
...
static void tech_send_ack(nua_handle_t *nh, private_object_t *tech_pvt, const char *r_sdp)
{
	...
	if (sofia_test_pflag(tech_pvt->profile, PFLAG_TRACK_CALLS)) {
		...
		nua_ack(nh, TAG_IF(invite_full_from, SIPTAG_FROM_STR(invite_full_from)),
				...
				TAG_IF(r_sdp && soa, SOATAG_USER_SDP_STR(r_sdp)), TAG_IF(r_sdp && soa, SOATAG_REUSE_REJECTED(1)),
				TAG_IF(r_sdp && soa, SOATAG_AUDIO_AUX("cn telephone-event")),
				TAG_IF(r_sdp && !soa, SIPTAG_CONTENT_TYPE_STR("application/sdp")),
				TAG_IF(r_sdp && !soa, SIPTAG_PAYLOAD_STR(r_sdp)), TAG_IF(r_sdp && !soa, NUTAG_MEDIA_ENABLE(0)),
				TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)), TAG_END());
	} else {
		nua_ack(nh, TAG_IF(invite_full_from, SIPTAG_FROM_STR(invite_full_from)),
				TAG_IF(invite_full_to, SIPTAG_TO_STR(invite_full_to)),
...
static void notify_watched_header(switch_core_session_t *session, const char *msgline, const char *hdrname,
								  const char *hdrval)
{
	switch_event_t *event = NULL;
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
					  "Found known watched header in message '%s', %s: %s\n", msgline, hdrname, hdrval);
	if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, MY_EVENT_NOTIFY_WATCHED_HEADER) ==
		SWITCH_STATUS_SUCCESS) {
		switch_channel_t *channel = switch_core_session_get_channel(session);
...
static void sofia_handle_sip_r_refer(nua_t *nua, sofia_profile_t *profile, nua_handle_t *nh,
									 switch_core_session_t *session, int status, const char *phrase, sip_t const *sip,
...
static void our_sofia_event_callback(nua_event_t event, int status, char const *phrase, nua_t *nua,
									 sofia_profile_t *profile, nua_handle_t *nh, sofia_private_t *sofia_private,
									 sip_t const *sip, sofia_dispatch_event_t *de, tagi_t tags[])
{
	...
	/* sofia_private will be == &mod_sofia_globals.keep_private whenever a request is done with a new handle that has to
	  be freed whenever the request is done */
	...
	if (sofia_private && sofia_private != &mod_sofia_globals.destroy_private &&
		...
		if (!zstr(sofia_private->gateway_name)) {
			if (!(gateway = sofia_reg_find_gateway(sofia_private->gateway_name))) { return; }
			...
			if ((session = de->init_session)) {
				...
				if (status >= 180 && !*sofia_private->auth_gateway_name) {
					...
					if (!zstr(gwname)) { switch_set_string(sofia_private->auth_gateway_name, gwname); }
				}
				...
				if (tech_pvt->gateway_name) { gateway = sofia_reg_find_gateway(tech_pvt->gateway_name); }

				if (channel && switch_channel_down(channel)) {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
									  "Channel is already hungup.\n");
					goto done;
				}
			} else {
				/* we can't find the session it must be hanging up or something else, its too late to do anything with
				 * it. */
				return;
			}
		}
	...
	if (session && tech_pvt && tech_pvt->watch_headers && sip) {
		...
		if (h) {
			...
			for (h = h->sh_succ; h; h = h->sh_succ) {
				...
				if (!sh->sh_class->hc_name) { continue; }
	...
	if (sofia_test_pflag(profile, PFLAG_AUTH_ALL) && tech_pvt && tech_pvt->key && sip &&
		(event < nua_r_set_params || event > nua_r_authenticate)) {
		...
		if (channel) { switch_channel_set_variable(channel, "sip_authorized", "true"); }
	}

	if (sip && (status == 401 || status == 407)) {
		sofia_reg_handle_sip_r_challenge(status, phrase, nua, profile, nh, sofia_private, session, gateway, sip, de,
										 tags);
		goto done;
	}
	...
	if (sip && sip->sip_payload && sip->sip_payload->pl_data) {
		if (sip->sip_payload->pl_len != strlen(sip->sip_payload->pl_data)) {
			sip->sip_payload->pl_data =
				su_strndup(nua_handle_get_home(nh), sip->sip_payload->pl_data, sip->sip_payload->pl_len);
		}
	}
	...
	case nua_i_cancel:
		...
		break;
	case nua_r_cancel: {
		if (status > 299 && nh) { nua_handle_destroy(nh); }
	} break;
	case nua_i_ack: {
		if (channel && sip) {
			...
			if (sip->sip_payload && sip->sip_payload->pl_data) {
				if (sofia_test_flag(tech_pvt, TFLAG_PASS_ACK)) {
					...
					if (!zstr(tech_pvt->mparams.prev_sdp_str) &&
						strcmp(tech_pvt->mparams.prev_sdp_str, sip->sip_payload->pl_data)) {
	...
	case nua_i_publish:
		...
		break;
	case nua_i_register:
		// nua_respond(nh, SIP_200_OK, SIPTAG_CONTACT(sip->sip_contact), NUTAG_WITH_THIS_MSG(de->data->e_msg),
	...
	case nua_i_message: {
		...
		if (handle_message) {
			sofia_presence_handle_sip_i_message(status, phrase, nua, profile, nh, session, sofia_private, sip, de,
												tags);
		}
	} break;
	case nua_i_info: {
		...
		if (handle_info) { sofia_handle_sip_i_info(nua, profile, nh, session, sip, de, tags); }

	} break;
	case nua_i_update:
		if (session) { 
			switch_event_t *event = NULL;
			const char* payload_ptr = switch_stristr("m=video", sip->sip_payload->pl_data);
			if (payload_ptr){
				const char* tmp_ptr = switch_stristr("a=fmtp:", payload_ptr+8);
				if(tmp_ptr){
					const char *pt = switch_channel_get_variable(channel, "ptStartCode");
					char *codec_order[SWITCH_MAX_CODECS];
					char* src = (char*)tmp_ptr+7;
					switch_separate_string(src, ' ', codec_order, SWITCH_MAX_CODECS);
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "received nua_i_update, update ptcode = %s,invite ptcode =%s .\n",codec_order[0],pt);
					if (strcmp(codec_order[0],pt)){
						if (switch_event_create(&event, SWITCH_EVENT_CUSTOM) == SWITCH_STATUS_SUCCESS) {
		                                	char *uuid = switch_core_session_get_uuid(session);
                		                	switch_channel_t *channel = switch_core_session_get_channel(session);
                                			event->subclass_name = strdup("sofia::events");
                                			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Event-Subclass", event->subclass_name);
                                			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Action", "update");
                                			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "my_uuid", uuid);
                                			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "ptStartCode", pt);
                                			switch_event_fire(&event);
                                			//switch_channel_set_private(channel, "received_sip_update", "true");
											switch_channel_set_variable(channel, "ptStartCode", codec_order[0]);
                        			}
			...
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "received nua_i_update .\n");
			sofia_update_callee_id(session, profile, sip, SWITCH_TRUE); 
		}
	
		break;
	case nua_r_update:
		if (session && tech_pvt && locked) { sofia_clear_flag_locked(tech_pvt, TFLAG_UPDATING_DISPLAY); }
		break;
	case nua_r_refer:
		if (session) { sofia_handle_sip_r_refer(nua, profile, nh, session, status, phrase, sip, de, tags); }
		break;
	case nua_i_refer:
		if (session) {
			sofia_handle_sip_i_refer(nua, profile, nh, session, sip, de, tags);
		} else if (sip) {
			const char *req_user = NULL, *req_host = NULL, *action = NULL, *ref_by_user = NULL, *ref_to_user = NULL,
					   *ref_to_host = NULL;
			...
			if (sip->sip_refer_to) {
				...
				if (sip->sip_refer_to->r_url->url_params &&
					switch_stristr("method=", sip->sip_refer_to->r_url->url_params)) {
				...
				if ((refer_to = sip_header_as_string(nua_handle_home(nh), (void *)sip->sip_refer_to))) {
					if ((iparams = strchr(refer_to, ';'))) {
						*iparams++ = '\0';
						...
						if (!params || !switch_stristr("method=", params)) { params = iparams; }
					}

					if ((tmp = sofia_glue_get_url_from_contact(refer_to, 0))) { refer_to = tmp; }
				}
			...
			if (sip->sip_referred_by) {
				referred_by = sofia_glue_get_url_from_contact(
					sip_header_as_string(nua_handle_home(nh), (void *)sip->sip_referred_by), 0);
				ref_by_user = sip->sip_referred_by->b_url->url_user;
			} else if (sip->sip_to) {
				referred_by =
					sofia_glue_get_url_from_contact(sip_header_as_string(nua_handle_home(nh), (void *)sip->sip_to), 0);
			...
			if (switch_event_create(&event, SWITCH_EVENT_CALL_SETUP_REQ) == SWITCH_STATUS_SUCCESS) {
				...
				switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Request-Target", "sofia/%s/%s", profile->name,
										refer_to);
				...
				if (sip->sip_call_id && sip->sip_call_id->i_id) {
					switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Request-Call-ID",
												   sip->sip_call_id->i_id);
				}

				if (!zstr(referred_by)) {
					switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Request-Sender", "sofia/%s/%s", profile->name,
											referred_by);
				}

				switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "var_origination_caller_id_number",
											   ref_by_user);
				switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "var_origination_caller_id_name",
											   ref_by_user);
				switch_event_fire(&event);
			}

			{
				...
				const char *call_id, *full_from, *full_to, *full_via, *from_user = NULL, *from_host = NULL, *to_user,
																	  *to_host, *full_agent;
				char to_tag[13] = "";
				...
				while (ap) {
					switch_snprintf(accept_header + strlen(accept_header),
									sizeof(accept_header) - strlen(accept_header), "%s%s ", ap->ac_type,
									ap->ac_next ? "," : "");
					ap = ap->ac_next;
				}
				...
				sql = switch_mprintf(
					...
					"full_via,expires,user_agent,accept,profile_name,hostname,network_port,network_ip,version,orig_"
					"proto, full_to) "
					"values "
					"('%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q',%ld,'%q','%q','%q','%q','%d','%q',-1,'%q',"
					"'%q;tag=%q')",
					proto, from_user, from_host, to_user, to_host,
					profile->presence_hosts ? profile->presence_hosts : "", event_str, contact_str, call_id, full_from,
					full_via, (long)switch_epoch_time_now(NULL) + 60, full_agent, accept_header, profile->name,
					mod_sofia_globals.hostname, np.network_port, np.network_ip, orig_proto, full_to, to_tag);
	...
	case nua_i_media_error: {
		...
		if (sofia_private && sofia_private->call_id && sofia_private->network_ip && sofia_private->network_port) {
			...
			sql = switch_mprintf(
				"delete from sip_registrations where call_id='%q' and network_ip='%q' and network_port='%q'",
			...
			if (switch_event_create(&event, SWITCH_EVENT_PRESENCE_IN) == SWITCH_STATUS_SUCCESS) {
				...
				switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "user-agent",
											   (sip && sip->sip_user_agent) ? sip->sip_user_agent->g_string
																			: "unknown");
				switch_event_add_header(event, SWITCH_STACK_BOTTOM, "from", "%s@%s", sofia_private->user,
										sofia_private->realm);
			...
			if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, MY_EVENT_UNREGISTER) ==
				SWITCH_STATUS_SUCCESS) {
			...
			sofia_reg_check_socket(profile, sofia_private->call_id, sofia_private->network_ip,
	...
	if (tech_pvt && tech_pvt->want_event && event == tech_pvt->want_event) { tech_pvt->want_event = 0; }

	if (sofia_private && sofia_private->call_id) { check_destroy = 0; }
	...
	case nua_i_notify:
		...
		if (sip && sip->sip_event && !strcmp(sip->sip_event->o_type, "dialog") && sip->sip_event->o_params &&
			!strcmp(sip->sip_event->o_params[0], "sla")) {
			check_destroy = 0;
		}
	...
	if (check_destroy) {
		if (nh && ((sofia_private && sofia_private->destroy_nh) || !nua_handle_magic(nh))) {
			if (sofia_private) { nua_handle_bind(nh, NULL); }

			if (tech_pvt && (tech_pvt->nh == nh)) { tech_pvt->nh = NULL; }
	...
	if (sofia_private && sofia_private->destroy_me) {
		if (tech_pvt) { tech_pvt->sofia_private = NULL; }
	...
	if (locked && tech_pvt) { switch_mutex_unlock(tech_pvt->sofia_mutex); }

	if (session && session != de->session) { switch_core_session_rwunlock(session); }
}
...
void sofia_process_dispatch_event(sofia_dispatch_event_t **dep)
{
	...
	*dep = NULL;
	...
	our_sofia_event_callback(de->data->e_event, de->data->e_status, de->data->e_phrase, de->nua, de->profile, de->nh,
							 sofia_private, de->sip, de, (tagi_t *)de->data->e_tags);
...
void *SWITCH_THREAD_FUNC sofia_msg_thread_run(switch_thread_t *thread, void *obj)
{
	...
	for (my_id = 0; my_id < mod_sofia_globals.msg_queue_len; my_id++) {
		if (mod_sofia_globals.msg_queue_thread[my_id] == thread) { break; }
	}
...
void sofia_msg_thread_start(int idx)
{
	...
	if (idx >= mod_sofia_globals.max_msg_queues || idx >= SOFIA_MAX_MSG_QUEUE ||
		(idx < mod_sofia_globals.msg_queue_len && mod_sofia_globals.msg_queue_thread[idx])) {
		return;
	}
	...
	if (idx >= mod_sofia_globals.msg_queue_len) {
		...
		for (i = 0; i < mod_sofia_globals.msg_queue_len; i++) {
			if (!mod_sofia_globals.msg_queue_thread[i]) {
				...
				switch_thread_create(&mod_sofia_globals.msg_queue_thread[i], thd_attr, sofia_msg_thread_run,
									 mod_sofia_globals.msg_queue, mod_sofia_globals.pool);
			}
		}
	}
...
void sofia_queue_message(sofia_dispatch_event_t *de)
{
	...
	if (de->profile && sofia_test_pflag(de->profile, PFLAG_THREAD_PER_REG) && de->data->e_event == nua_i_register &&
		DE_THREAD_CNT < mod_sofia_globals.max_reg_threads) {
...
{
	if (!tech_pvt->call_id && tech_pvt->session && tech_pvt->channel && sip && sip->sip_call_id &&
		sip->sip_call_id->i_id) {
...
void sofia_event_callback(nua_event_t event, int status, char const *phrase, nua_t *nua, sofia_profile_t *profile,
						  nua_handle_t *nh, sofia_private_t *sofia_private, sip_t const *sip, tagi_t tags[])
{
				...
				if (switch_channel_direction(channel) == SWITCH_CALL_DIRECTION_INBOUND &&
					...
					if (status == 403) {
						...
						if (!zstr(tech_pvt->call_id)) {
							...
							switch_mutex_lock(profile->flag_mutex);
							switch_core_hash_insert_dup_auto_free(profile->chat_hash, tech_pvt->call_id,
																  switch_core_session_get_uuid(session));
							switch_mutex_unlock(profile->flag_mutex);
							nua_handle_destroy(nh);
						} else {
	...
	case nua_i_info:
		...
		if (!sofia_private) {
			if (sess_count >= sess_max || !sofia_test_pflag(profile, PFLAG_RUNNING) || !switch_core_ready_inbound()) {
				nua_respond(nh, 503, "Maximum Calls In Progress", SIPTAG_RETRY_AFTER_STR("300"), NUTAG_WITH_THIS(nua),
							TAG_END());
				nua_handle_destroy(nh);
				goto end;
			}
	...
	if (event == nua_i_invite && !sofia_private) {
		...
		if (!(sofia_private = su_alloc(nua_handle_get_home(nh), sizeof(*sofia_private)))) { abort(); }
		...
		if (sip->sip_call_id && sip->sip_call_id->i_id) {
			...
			if (uuid) {
				if ((session = switch_core_session_locate(uuid))) {
					tech_pvt = switch_core_session_get_private(session);
					switch_copy_string(sofia_private->uuid_str, switch_core_session_get_uuid(session),
									   sizeof(sofia_private->uuid_str));
		...
		if (sofia_test_pflag(profile, PFLAG_CALLID_AS_UUID)) {
			session = switch_core_session_request_uuid(sofia_endpoint_interface, SWITCH_CALL_DIRECTION_INBOUND,
													   SOF_NONE, NULL, sip->sip_call_id->i_id);
		} else {
			session =
		...
		if (session) {
			...
			if (sip->sip_from) { channel_name = url_set_chanvars(session, sip->sip_from->a_url, sip_from); }
...
void event_handler(switch_event_t *event)
{
	...
	if (!strcasecmp(class, "PRESENCE_IN")) {
		...
		if (switch_event_create(&pevent, SWITCH_EVENT_PRESENCE_IN) == SWITCH_STATUS_SUCCESS) {
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "alt_event_type",
										   switch_event_get_header_nil(event, "orig-alt_event_type"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "answer-state",
										   switch_event_get_header_nil(event, "orig-answer-state"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "astate",
										   switch_event_get_header_nil(event, "orig-astate"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "call-direction",
										   switch_event_get_header_nil(event, "orig-call-direction"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "Caller-Callee-ID-Number",
										   switch_event_get_header_nil(event, "Orig-Caller-Callee-ID-Number"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "Caller-Caller-ID-Name",
										   switch_event_get_header_nil(event, "Orig-Caller-Caller-ID-Name"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "Caller-Caller-ID-Number",
										   switch_event_get_header_nil(event, "Orig-Caller-Caller-ID-Number"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "Caller-Destination-Number",
										   switch_event_get_header_nil(event, "Orig-Caller-Destination-Number"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "Caller-Direction",
										   switch_event_get_header_nil(event, "Orig-Caller-Direction"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "Caller-Username",
										   switch_event_get_header_nil(event, "Orig-Caller-Username"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "channel-state",
										   switch_event_get_header_nil(event, "orig-channel-state"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "force-status",
										   switch_event_get_header_nil(event, "orig-force-status"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "from",
										   switch_event_get_header_nil(event, "orig-from"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "login",
										   switch_event_get_header_nil(event, "orig-login"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "Other-Leg-Caller-ID-Name",
										   switch_event_get_header_nil(event, "Orig-Other-Leg-Caller-ID-Name"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "Other-Leg-Caller-ID-Number",
										   switch_event_get_header_nil(event, "Orig-Other-Leg-Caller-ID-Number"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "presence-call-direction",
										   switch_event_get_header_nil(event, "orig-presence-call-direction"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "presence-call-info-state",
										   switch_event_get_header_nil(event, "orig-presence-call-info-state"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "Presence-Privacy",
										   switch_event_get_header_nil(event, "Orig-Presence-Privacy"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "proto",
										   switch_event_get_header_nil(event, "orig-proto"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "register-source",
										   switch_event_get_header_nil(event, "orig-register-source"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "resub",
										   switch_event_get_header_nil(event, "orig-resub"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "rpid",
										   switch_event_get_header_nil(event, "orig-rpid"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "status",
										   switch_event_get_header_nil(event, "orig-status"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "Unique-ID",
										   switch_event_get_header_nil(event, "Orig-Unique-ID"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "variable_sip_from_user",
										   switch_event_get_header_nil(event, "Orig-variable_sip_from_user"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "variable_sip_to_user",
										   switch_event_get_header_nil(event, "Orig-variable_sip_to_user"));
		...
		if (switch_event_create(&pevent, SWITCH_EVENT_MESSAGE_WAITING) == SWITCH_STATUS_SUCCESS) {
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "MWI-Messages-Waiting",
										   switch_event_get_header_nil(event, "orig-MWI-Messages-Waiting"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "MWI-Message-Account",
										   switch_event_get_header_nil(event, "orig-MWI-Message-Account"));
			switch_event_add_header_string(pevent, SWITCH_STACK_BOTTOM, "MWI-Voice-Message",
										   switch_event_get_header_nil(event, "orig-MWI-Voice-Message"));
			/* we cannot use switch_event_fire, or otherwise we'll start an endless loop */
			sofia_presence_event_handler(pevent);
			return;
		} else {
	...
	} else if ((subclass = switch_event_get_header_nil(event, "orig-event-subclass")) &&
			   !strcasecmp(subclass, MY_EVENT_UNREGISTER)) {
		...
		if (sofia_test_pflag(profile, PFLAG_MULTIREG)) {
			...
			sql = switch_mprintf("delete from sip_registrations where sip_user='%q' and sip_host='%q'", from_user,
								 from_host);
		}
		...
		sofia_glue_release_profile(profile);
			   ...
			   !strcasecmp(subclass, MY_EVENT_REGISTER)) {
		char *from_user = switch_event_get_header_nil(event, "orig-from-user");
		char *from_host = switch_event_get_header_nil(event, "orig-from-host");
		...
		if (!mwi_user) { mwi_user = (char *)from_user; }
		if (!mwi_host) { mwi_host = (char *)from_host; }

		if (exp_str) { expires += atol(exp_str); }

		if (!rpid) { rpid = "unknown"; }
		...
		if (mod_sofia_globals.rewrite_multicasted_fs_path && contact_str) {
			...
			if ((sptr = strstr(fixed_contact_str, needle))) {
				...
				if (origsptr) { eptr = strchr(++origsptr, ';'); }
			} else {
			...
			switch (mod_sofia_globals.rewrite_multicasted_fs_path) {
			case 1:
				switch_snprintf(sptr, len - (sptr - fixed_contact_str), ";fs_path=sip:%s%s", to_host,
								eptr ? eptr : ">");
				break;
			case 2:
				switch_snprintf(sptr, len - (sptr - fixed_contact_str), ";fs_path=sip:%s%s", orig_server_host,
								eptr ? eptr : ">");
				break;
			case 3:
				switch_snprintf(sptr, len - (sptr - fixed_contact_str), ";fs_path=sip:%s%s", orig_hostname,
								eptr ? eptr : ">");
				break;
			default:
				switch_snprintf(sptr, len - (sptr - fixed_contact_str), ";fs_path=sip:%s%s", to_host,
		...
		sql = switch_mprintf("insert into sip_registrations "
							 ...
							 "sip_username, sip_realm,"
							 "mwi_user, mwi_host, orig_server_host, orig_hostname, ping_status, ping_count) "
							 "values ('%q','%q','%q','%q','%q','Registered','%q',%ld, "
							 "'%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q', '%q', %d)",
							 call_id, from_user, from_host, presence_hosts, contact_str, rpid, expires, user_agent,
							 to_user, guess_ip4, profile_name, mod_sofia_globals.hostname, network_ip, network_port,
							 username, realm, mwi_user, mwi_host, orig_server_host, orig_hostname, "Reachable", 0);
		...
		switch_safe_free(dup_mwi_account);
			   ...
			   !strcasecmp(subclass, MY_EVENT_SIP_USER_STATE)) {
		char *profile_name = switch_event_get_header_nil(event, "orig-profile-name");
		char *from_user = switch_event_get_header_nil(event, "orig-from-user");
		char *from_host = switch_event_get_header_nil(event, "orig-from-host");
		...
		if (!profile_name || !(profile = sofia_glue_find_profile(profile_name))) {
			...
			if (!strcmp(ping_status, "REACHABLE")) {
				sql = switch_mprintf("update sip_registrations set ping_status='%q' where sip_user='%q' and "
									 "sip_host='%q' and call_id='%q'",
			...
			if (sql) {
				sofia_glue_execute_sql(profile, &sql, SWITCH_TRUE);
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
								  "Propagating sip_user_state for %s@%s. Ping-Status: %s\n", from_user, from_host,
								  ping_status);
			}
	...
	if (profile) {
		if (!strcasecmp(profile->shutdown_type, "true")) {
			...
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
							  "Profile %s could not load! Waiting for calls to finish, then shutting down!\n",
							  profile->name);
			...
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
							  "Profile %s could not load! Asking for reincarnation now!\n", profile->name);
...
void watchdog_triggered_abort(void) { abort(); }
...
void *SWITCH_THREAD_FUNC sofia_profile_worker_thread_run(switch_thread_t *thread, void *obj)
{
	...
	uint32_t ireg_loops =
		profile->ireg_seconds; /* Number of loop iterations done when we haven't checked for registrations */
	uint32_t iping_loops =
		profile->iping_freq; /* Number of loop iterations done when we haven't checked for ping expires */
		...
		if (tick) {
			if (profile->watchdog_enabled) {
				...
				if (profile->step_timeout) {
					...
					if (step_diff > profile->step_timeout) { step_fail = 1; }
				}

				if (profile->event_timeout) {
					event_diff = (uint32_t)((switch_time_now() - profile->last_sip_event) / 1000);

					if (event_diff > profile->event_timeout) { event_fail = 1; }
				}

				if (step_fail && profile->event_timeout && !event_fail) { step_fail = 0; }
...
switch_thread_t *launch_sofia_worker_thread(sofia_profile_t *profile)
{
	...
	switch_threadattr_t *thd_attr = NULL;
	int x = 0;
	switch_xml_t cfg = NULL, xml = NULL, xprofile = NULL, xprofiles = NULL, gateways_tag = NULL, domains_tag = NULL,
				 domain_tag = NULL;
				...
				for (domain_tag = switch_xml_child(domains_tag, "domain"); domain_tag; domain_tag = domain_tag->next) {
					...
					if (!zstr(dname)) {
						if (!strcasecmp(dname, "all")) {
							...
							if (switch_xml_locate("directory", NULL, NULL, NULL, &xml_root, &x_domains, xml_params,
												  SWITCH_FALSE) == SWITCH_STATUS_SUCCESS) {
								for (x_domain_tag = switch_xml_child(x_domains, "domain"); x_domain_tag;
									 x_domain_tag = x_domain_tag->next) {
									dname = switch_xml_attr_soft(x_domain_tag, "name");
									parse_domain_tag(profile, x_domain_tag, dname, parse, alias);
								}
								switch_xml_free(xml_root);
							}
						} else if (switch_xml_locate_domain(dname, xml_params, &droot, &x_domain_tag) ==
								   SWITCH_STATUS_SUCCESS) {
							parse_domain_tag(profile, x_domain_tag, dname, parse, alias);
...
void *SWITCH_THREAD_FUNC sofia_profile_thread_run(switch_thread_t *thread, void *obj)
{
	...
	supported = switch_core_sprintf(profile->pool, "%s%s%spath, replaces", use_100rel ? "100rel, " : "",
	...
	if (sofia_test_pflag(profile, PFLAG_AUTO_NAT) && switch_nat_get_type()) {
		if ((!sofia_test_pflag(profile, PFLAG_TLS) || !profile->tls_only) &&
			switch_nat_add_mapping(profile->sip_port, SWITCH_NAT_UDP, NULL, SWITCH_FALSE) == SWITCH_STATUS_SUCCESS) {
		...
		if (sofia_test_pflag(profile, PFLAG_TLS) && switch_nat_add_mapping(profile->tls_sip_port, SWITCH_NAT_TCP, NULL,
																		   SWITCH_FALSE) == SWITCH_STATUS_SUCCESS) {
	...
	/* We have to init the verify_subjects here as during config stage profile->home isn't setup, it should be freed
	 * when profile->home is freed */
	if ((profile->tls_verify_policy & TPTLS_VERIFY_SUBJECTS_IN) && profile->tls_verify_in_subjects_str &&
		!profile->tls_verify_in_subjects) {
		profile->tls_verify_in_subjects =
			su_strlst_dup_split((su_home_t *)profile->nua, profile->tls_verify_in_subjects_str, "|");
	}
	...
	if (!SSL_CTX_use_certificate_chain_file(ssl_ctx, chain)) { ssl_error = SWITCH_TRUE; }

	if (!ssl_error && !SSL_CTX_use_certificate_file(ssl_ctx, cert, SSL_FILETYPE_PEM)) { ssl_error = SWITCH_TRUE; }

	if (!ssl_error && !SSL_CTX_use_PrivateKey_file(ssl_ctx, key, SSL_FILETYPE_PEM)) { ssl_error = SWITCH_TRUE; }

	if (!ssl_error && !SSL_CTX_check_private_key(ssl_ctx)) { ssl_error = SWITCH_TRUE; }

	if (ssl_error) { attempts = profile->bind_attempts; }

	SSL_CTX_free(ssl_ctx);
#endif

	do {
		profile->nua = nua_create(
			profile->s_root,	  /* Event loop */
			sofia_event_callback, /* Callback for processing events */
			profile,			  /* Additional data to pass to callback */
			TAG_IF(!sofia_test_pflag(profile, PFLAG_TLS) || !profile->tls_only, NUTAG_URL(profile->bindurl)),
			NTATAG_USER_VIA(1), TPTAG_PONG2PING(1), NTATAG_TCP_RPORT(0), NTATAG_TLS_RPORT(0),
...
#endif
			NUTAG_RETRY_AFTER_ENABLE(0), NUTAG_AUTO_INVITE_100(0),
			...
			TAG_IF(profile->ws_bindurl, NUTAG_WS_URL(profile->ws_bindurl)),
			TAG_IF(profile->wss_bindurl, NUTAG_WSS_URL(profile->wss_bindurl)),
			TAG_IF(profile->tls_cert_dir, NUTAG_CERTIFICATE_DIR(profile->tls_cert_dir)),
			TAG_IF(sofia_test_pflag(profile, PFLAG_TLS) && profile->tls_passphrase,
				   TPTAG_TLS_PASSPHRASE(profile->tls_passphrase)),
			TAG_IF(sofia_test_pflag(profile, PFLAG_TLS), TPTAG_TLS_VERIFY_POLICY(profile->tls_verify_policy)),
			TAG_IF(sofia_test_pflag(profile, PFLAG_TLS), TPTAG_TLS_VERIFY_DEPTH(profile->tls_verify_depth)),
			TAG_IF(sofia_test_pflag(profile, PFLAG_TLS), TPTAG_TLS_VERIFY_DATE(profile->tls_verify_date)),
			...
			TAG_IF(sofia_test_pflag(profile, PFLAG_TLS), TPTAG_TLS_CIPHERS(profile->tls_ciphers)),
			...
			TAG_IF(sofia_test_pflag(profile, PFLAG_TCP_PINGPONG), TPTAG_PINGPONG(profile->tcp_pingpong)),
			TAG_IF(sofia_test_pflag(profile, PFLAG_TCP_PING2PONG), TPTAG_PINGPONG(profile->tcp_ping2pong)),
			TAG_IF(sofia_test_pflag(profile, PFLAG_DISABLE_SRV503), NTATAG_SRV_503(0)),
			TAG_IF(sofia_test_pflag(profile, PFLAG_SOCKET_TCP_KEEPALIVE),
				   TPTAG_SOCKET_KEEPALIVE(profile->socket_tcp_keepalive)),
			TAG_IF(sofia_test_pflag(profile, PFLAG_TCP_KEEPALIVE), TPTAG_KEEPALIVE(profile->tcp_keepalive)),
			NTATAG_DEFAULT_PROXY(profile->outbound_proxy), NTATAG_SERVER_RPORT(profile->server_rport_level),
			NTATAG_CLIENT_RPORT(profile->client_rport_level), TPTAG_LOG(sofia_test_flag(profile, TFLAG_TPORT_LOG)),
			...
			TAG_IF(sofia_test_pflag(profile, PFLAG_NO_CONNECTION_REUSE), TPTAG_REUSE(0)),
			TAG_END()); /* Last tag should always finish the sequence */

		if (!ssl_error && !profile->nua) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
							  "Error Creating SIP UA for profile: %s (%s) ATTEMPT %d (RETRY IN %d SEC)\n",
							  profile->name, profile->bindurl, attempts + 1, profile->bind_attempt_interval);
			if (attempts < profile->bind_attempts) { switch_yield(1000000 * profile->bind_attempt_interval); }
		}
	...
	if (!profile->nua) {
		if (!ssl_error) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
							  "Error Creating SIP UA for profile: %s (%s)\n"
	...
	nua_set_params(
		profile->nua, SIPTAG_ALLOW_STR("INVITE, ACK, BYE, CANCEL, OPTIONS, MESSAGE, INFO"), SIPTAG_USER_AGENT(SIP_NONE),
		NUTAG_AUTOANSWER(0), NUTAG_AUTOACK(0), NUTAG_AUTOALERT(0), NUTAG_ENABLEMESSENGER(1), NTATAG_EXTRA_100(0),
		...
		TAG_IF(!sofia_test_pflag(profile, PFLAG_DISABLE_100REL), NUTAG_ALLOW("PRACK")), NUTAG_ALLOW("INFO"),
		NUTAG_ALLOW("NOTIFY"), NUTAG_ALLOW_EVENTS("talk"), NUTAG_ALLOW_EVENTS("hold"), NUTAG_ALLOW_EVENTS("conference"),
		NUTAG_APPL_METHOD("OPTIONS"), NUTAG_APPL_METHOD("INVITE"), NUTAG_APPL_METHOD("REFER"),
		NUTAG_APPL_METHOD("REGISTER"), NUTAG_APPL_METHOD("NOTIFY"), NUTAG_APPL_METHOD("INFO"), NUTAG_APPL_METHOD("ACK"),
		...
		TAG_IF(profile->session_timeout && profile->minimum_session_expires,
			   NUTAG_MIN_SE(profile->minimum_session_expires)),
		NUTAG_SESSION_TIMER(profile->session_timeout), NTATAG_MAX_PROCEEDING(profile->max_proceeding),
		...
		TAG_IF(profile->pres_type, NUTAG_ENABLEMESSAGE(1)), TAG_IF(profile->pres_type, NUTAG_ALLOW_EVENTS("presence")),
		TAG_IF(profile->pres_type, NUTAG_ALLOW_EVENTS("as-feature-event")),
		TAG_IF((profile->pres_type || sofia_test_pflag(profile, PFLAG_MANAGE_SHARED_APPEARANCE)),
		...
		TAG_IF(profile->pres_type == PRES_TYPE_PNP, NUTAG_ALLOW_EVENTS("ua-profile")), NUTAG_ALLOW_EVENTS("refer"),
		SIPTAG_SUPPORTED_STR(supported),
		TAG_IF(strcasecmp(profile->user_agent, "_undef_"), SIPTAG_USER_AGENT_STR(profile->user_agent)), TAG_END());
	...
	for (node = profile->aliases; node; node = node->next) {
		node->nua = nua_create(profile->s_root,		 /* Event loop */
							   sofia_event_callback, /* Callback for processing events */
							   profile,				 /* Additional data to pass to callback */
							   NTATAG_SERVER_RPORT(profile->server_rport_level), NUTAG_URL(node->url),
							   TAG_END()); /* Last tag should always finish the sequence */

		nua_set_params(node->nua, SIPTAG_USER_AGENT(SIP_NONE), NUTAG_APPL_METHOD("OPTIONS"), NUTAG_APPL_METHOD("REFER"),
					   NUTAG_APPL_METHOD("SUBSCRIBE"), NUTAG_AUTOANSWER(0), NUTAG_AUTOACK(0), NUTAG_AUTOALERT(0),
					   TAG_IF((profile->mflags & MFLAG_REGISTER), NUTAG_ALLOW("REGISTER")),
					   TAG_IF((profile->mflags & MFLAG_REFER), NUTAG_ALLOW("REFER")), NUTAG_ALLOW("INFO"),
					   TAG_IF(profile->pres_type, NUTAG_ALLOW("PUBLISH")),
					   TAG_IF(profile->pres_type, NUTAG_ENABLEMESSAGE(1)), SIPTAG_SUPPORTED_STR(supported),
	...
	switch_sql_queue_manager_init_name(qname, &profile->qm, 2, profile->odbc_dsn ? profile->odbc_dsn : profile->dbname,
									   SWITCH_MAX_TRANS, profile->pre_trans_execute, profile->post_trans_execute,
									   profile->inner_pre_trans_execute, profile->inner_post_trans_execute);
	...
	while (mod_sofia_globals.running == 1 && sofia_test_pflag(profile, PFLAG_RUNNING) &&
		   sofia_test_pflag(profile, PFLAG_WORKER_RUNNING)) {
...
nua_fail:
	if (sofia_test_pflag(profile, PFLAG_AUTO_NAT) && switch_nat_get_type()) {
		...
		if (sofia_test_pflag(profile, PFLAG_TLS) &&
			switch_nat_del_mapping(profile->tls_sip_port, SWITCH_NAT_TCP) == SWITCH_STATUS_SUCCESS) {
...
db_fail:
	...
	if (sofia_test_pflag(profile, PFLAG_RESPAWN)) { config_sofia(SOFIA_CONFIG_RESPAWN, profile->name); }
...
switch_status_t sofia_set_loglevel(const char *name, int level)
{
	...
	if (level < 0 || level > 9) { return SWITCH_STATUS_FALSE; }
	...
	if (!(log = sofia_get_logger(name))) { return SWITCH_STATUS_FALSE; }
...
static void parse_gateway_subscriptions(sofia_profile_t *profile, sofia_gateway_t *gateway, switch_xml_t gw_subs_tag)
{
	...
	for (subscription_tag = switch_xml_child(gw_subs_tag, "subscription"); subscription_tag;
		 subscription_tag = subscription_tag->next) {
		...
		if ((gw_sub = switch_core_alloc(profile->pool, sizeof(*gw_sub)))) {
			...
			if (gw_sub->retry_seconds < 10) {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
								  "INVALID: retry_seconds correcting the value to 30\n");
...
static void parse_gateways(sofia_profile_t *profile, switch_xml_t gateways_tag, const char *gwname)
{
	...
	for (gateway_tag = switch_xml_child(gateways_tag, "gateway"); gateway_tag; gateway_tag = gateway_tag->next) {
		...
		if (switch_core_hash_find(mod_sofia_globals.gateway_hash, name) &&
			(gp = switch_core_hash_find(mod_sofia_globals.gateway_hash, pkey)) && !gp->deleted) {
		...
		if ((gateway = switch_core_alloc(profile->pool, sizeof(*gateway)))) {
			...
			switch_uuid_t uuid;
			uint32_t ping_freq = 0, extension_in_contact = 0, contact_in_ping = 0, ping_monitoring = 0, distinct_to = 0,
					 rfc_5626 = 0;
			int ping_max = 1, ping_min = 1;
			char *register_str = "true", *scheme = "Digest", *realm = NULL, *username = NULL, *auth_username = NULL,
				 *password = NULL, *caller_id_in_from = "false", *extension = NULL, *proxy = NULL,
				 *options_user_agent = NULL, *context = profile->context, *expire_seconds = "3600",
				 *retry_seconds = "30", *fail_908_retry_seconds = NULL, *timeout_seconds = "60", *from_user = "",
				 *from_domain = NULL, *outbound_proxy = NULL, *register_proxy = NULL, *contact_host = NULL,
				 *contact_params = "", *params = NULL, *register_transport = NULL, *reg_id = NULL, *str_rfc_5626 = "";
			...
			for (; param; param = param->next) {
				...
				if (var && val) {
					...
					if (in) {
						if (!gateway->ib_vars) { switch_event_create_plain(&gateway->ib_vars, SWITCH_EVENT_GENERAL); }
						switch_event_add_header_string(gateway->ib_vars, SWITCH_STACK_BOTTOM, var, val);
					}

					if (out) {
						if (!gateway->ob_vars) { switch_event_create_plain(&gateway->ob_vars, SWITCH_EVENT_GENERAL); }
			...
			for (; param; param = param->next) {
				...
				if (!strcmp(var, "register")) {
				...
				} else if (!strcmp(var, "ping-monitoring")) { // if true then every gw ping result will fire a gateway
															  // status event
					...
					if (!zstr(val)) { gateway->destination_prefix = switch_core_strdup(gateway->pool, val); }
					...
					if (transport == SOFIA_TRANSPORT_UNKNOWN ||
						(!sofia_test_pflag(profile, PFLAG_TLS) && sofia_glue_transport_has_tls(transport))) {
					...
					if (!zstr(val)) { gateway->gw_auth_acl = switch_core_strdup(gateway->pool, val); }
				}
			}
			...
			if (rfc_5626 && sofia_test_pflag(profile, PFLAG_ENABLE_RFC5626)) {
				...
				str_rfc_5626 =
					switch_core_sprintf(gateway->pool, ";reg-id=%s;+sip.instance=\"<urn:uuid:%s>\"", reg_id, str_guid);
			}
			...
			if (switch_true(register_str)) {
				...
				if (zstr(username)) { username = "FreeSWITCH"; }

				if (zstr(password)) { password = ""; }
			}

			if (zstr(from_user)) { from_user = username; }

			if (zstr(proxy)) { proxy = realm; }
			...
			if (zstr(auth_username)) { auth_username = username; }
			...
			if (fail_908_retry_seconds) { gateway->fail_908_retry_seconds = atoi(fail_908_retry_seconds); }
			...
			if (switch_true(caller_id_in_from)) { sofia_set_flag(gateway, REG_FLAG_CALLERID); }

			register_transport = (char *)sofia_glue_transport2str(gateway->register_transport);

			if (!zstr(contact_params)) {
				if (*contact_params == ';') {
					params = switch_core_sprintf(gateway->pool, "%s;transport=%s;gw=%s", contact_params,
												 register_transport, gateway->name);
				} else {
					params = switch_core_sprintf(gateway->pool, ";%s;transport=%s;gw=%s", contact_params,
												 register_transport, gateway->name);
				}
			...
			if (!zstr(from_domain)) { gateway->from_domain = switch_core_strdup(gateway->pool, from_domain); }

			if (!zstr(register_transport) && !switch_stristr("transport=", proxy)) {
				gateway->register_url =
					switch_core_sprintf(gateway->pool, "sip:%s;transport=%s", proxy, register_transport);
			} else {
				gateway->register_url = switch_core_sprintf(gateway->pool, "sip:%s", proxy);
			}

			gateway->register_from =
				switch_core_sprintf(gateway->pool, "<sip:%s@%s>", from_user, !zstr(from_domain) ? from_domain : proxy);
			...
			if (ping_freq) {
				if (ping_freq >= 5) {
					...
					gateway->options_to_uri =
						switch_core_sprintf(gateway->pool, "<sip:%s>", !zstr(from_domain) ? from_domain : proxy);
					gateway->options_from_uri = gateway->options_to_uri;
					if (contact_in_ping) { gateway->contact_in_ping = contact_in_ping; }
				} else {
			...
			if (sofia_test_pflag(profile, PFLAG_AUTO_NAT)) {
				...
				if (register_host && switch_is_lan_addr(register_host)) { sipip = profile->sipip; }
			...
			if (extension_in_contact) {
				if (rfc_5626) {
					...
					gateway->register_contact = switch_core_sprintf(
						gateway->pool, format, extension, sipip,
						sofia_glue_transport_has_tls(gateway->register_transport) ? profile->tls_sip_port
																				  : profile->extsipport,
						params, str_rfc_5626);
				...
				if (rfc_5626) {
					...
					gateway->register_contact = switch_core_sprintf(
						gateway->pool, format, gateway->name, sipip,
						sofia_glue_transport_has_tls(gateway->register_transport) ? profile->tls_sip_port
																				  : profile->extsipport,
						params, str_rfc_5626);
							 ...
							 const char *alias)
{
	...
	if (switch_true(parse)) {
		...
		for (ut = switch_xml_child(x_domain_tag, "user"); ut; ut = ut->next) {
			if (((gateways_tag = switch_xml_child(ut, "gateways")))) { parse_gateways(profile, gateways_tag, NULL); }
		}
...
static void config_sofia_profile_urls(sofia_profile_t *profile)
{
	...
	if (profile->extsipip) {
		...
		profile->public_url =
			switch_core_sprintf(profile->pool, "sip:%s@%s%s%s:%d", profile->contact_user, ipv6 ? "[" : "",
								profile->extsipip, ipv6 ? "]" : "", profile->extsipport);
	}

	if (profile->extsipip && !sofia_test_pflag(profile, PFLAG_AUTO_NAT)) {
		char *ipv6 = strchr(profile->extsipip, ':');
		profile->url = switch_core_sprintf(profile->pool, "sip:%s@%s%s%s:%d", profile->contact_user, ipv6 ? "[" : "",
										   profile->extsipip, ipv6 ? "]" : "", profile->extsipport);
		profile->bindurl = switch_core_sprintf(profile->pool, "%s;maddr=%s", profile->url, profile->sipip);
	} else {
		char *ipv6 = strchr(profile->sipip, ':');
		profile->url = switch_core_sprintf(profile->pool, "sip:%s@%s%s%s:%d", profile->contact_user, ipv6 ? "[" : "",
										   profile->sipip, ipv6 ? "]" : "", profile->sip_port);
	...
	if (profile->ws_port) {
		...
		char *ipv6 = strchr(ip, ':');
		profile->ws_bindurl = switch_core_sprintf(profile->pool, "sip:%s@%s%s%s:%d;transport=ws", profile->contact_user,
												  ipv6 ? "[" : "", ip, ipv6 ? "]" : "", port);
	}
	...
	if (profile->wss_port) {
		...
		char *ipv6 = strchr(ip, ':');
		profile->wss_bindurl = switch_core_sprintf(profile->pool, "sips:%s@%s%s%s:%d;transport=wss",
	...
	if (sofia_test_pflag(profile, PFLAG_TLS)) {
		...
		if (profile->extsipip) {
			char *ipv6 = strchr(profile->extsipip, ':');
			profile->tls_public_url =
				switch_core_sprintf(profile->pool, "sip:%s@%s%s%s:%d", profile->contact_user, ipv6 ? "[" : "",
									profile->extsipip, ipv6 ? "]" : "", profile->tls_sip_port);
		}
		...
		if (profile->extsipip && !sofia_test_pflag(profile, PFLAG_AUTO_NAT)) {
			...
			profile->tls_url =
				switch_core_sprintf(profile->pool, "sip:%s@%s%s%s:%d", profile->contact_user, ipv6 ? "[" : "",
									profile->extsipip, ipv6 ? "]" : "", profile->tls_sip_port);
			profile->tls_bindurl =
				switch_core_sprintf(profile->pool, "sips:%s@%s%s%s:%d;maddr=%s", profile->contact_user, ipv6 ? "[" : "",
									profile->extsipip, ipv6 ? "]" : "", profile->tls_sip_port, profile->sipip);
		} else {
			char *ipv6 = strchr(profile->sipip, ':');
			profile->tls_url =
				switch_core_sprintf(profile->pool, "sip:%s@%s%s%s:%d", profile->contact_user, ipv6 ? "[" : "",
									profile->sipip, ipv6 ? "]" : "", profile->tls_sip_port);
			profile->tls_bindurl =
				switch_core_sprintf(profile->pool, "sips:%s@%s%s%s:%d", profile->contact_user, ipv6 ? "[" : "",
									profile->sipip, ipv6 ? "]" : "", profile->tls_sip_port);
		}
		...
		if (profile->tls_public_url) {
			profile->tls_public_contact =
				switch_core_sprintf(profile->pool, "<%s;transport=tls>", profile->tls_public_url);
		}
	}
}
...
switch_status_t config_sofia(sofia_config_t reload, char *profile_name)
{
	...
	mod_sofia_globals.reg_deny_binding_fetch_and_no_lookup =
		SWITCH_FALSE; /* handle backwards compatilibity - by default use new behavior */
	...
	if ((settings = switch_xml_child(cfg, "global_settings"))) {
		for (param = switch_xml_child(settings, "param"); param; param = param->next) {
			...
			if (!strcasecmp(var, "log-level")) {
				...
				if (x > 0) { mod_sofia_globals.max_reg_threads = x; }
				...
				mod_sofia_globals.reg_deny_binding_fetch_and_no_lookup =
					switch_true(val); /* remove when noone complains about the extra lookup */
				if (switch_true(val)) {
					switch_log_printf(
	...
	if ((profiles = switch_xml_child(cfg, "profiles"))) {
		for (xprofile = switch_xml_child(profiles, "profile"); xprofile; xprofile = xprofile->next) {
			...
			if (!(settings = switch_xml_child(xprofile, "settings"))) {
				...
				if (!xprofilename) { xprofilename = "unnamed"; }
				...
				if (!profile_already_started) {
					...
					profile->auto_rtp_bugs =
						RTP_BUG_CISCO_SKIP_MARK_BIT_2833; // | RTP_BUG_SONUS_SEND_INVALID_TIMESTAMP_2833;

					profile->pool = pool;

					profile->user_agent =
						switch_core_sprintf(profile->pool, "FreeSWITCH-mod_sofia/%s", switch_version_full());
					...
					if (xprofiledomain) { profile->domain_name = switch_core_strdup(profile->pool, xprofiledomain); }
				...
				for (param = switch_xml_child(settings, "param"); param; param = param->next) {
					...
					char *val = (char *)switch_xml_attr_soft(param, "value");
					int found =
						1; // Used to break up long if/elseif chain (MSVC2015 fails (parser stack overflow) otherwise)
					...
					if (!strcasecmp(var, "debug") && val) {
						...
						if (ba >= 0) { profile->bind_attempts = ba; }
					} else if (!strcasecmp(var, "bind-attempt-interval") && val) {
						int bai = atoi(val);

						if (bai >= 0) { profile->bind_attempt_interval = bai; }
					...
					} else if ((!strcasecmp(var, "proxy-refer-replaces") || !strcasecmp(var, "proxy-refer")) &&
							   !zstr(val)) {
						if (switch_true(val)) {
						...
						if (profile->ireg_seconds < 0) { profile->ireg_seconds = IREG_SECONDS; }
					} else if (!strcasecmp(var, "ping-mean-interval") && !zstr(val)) {
						profile->iping_seconds = atoi(val);
						if (profile->iping_seconds < 0) { profile->iping_seconds = IPING_SECONDS; }
					} else if (!strcasecmp(var, "ping-thread-frequency") && !zstr(val)) {
						profile->iping_freq = atoi(val);
						if (profile->iping_freq < 0) { profile->iping_freq = IPING_FREQUENCY; }
						...
						int msec = atoi(val);
						if (msec > 19) { profile->jb_msec = switch_core_strdup(profile->pool, val); }
						...
						int tmp = atoi(val);
						if (tmp > 0) { profile->force_subscription_expires = tmp; }
					} else if (!strcasecmp(var, "force-publish-expires") && !zstr(val)) {
						int tmp = atoi(val);
						if (tmp > 0) { profile->force_publish_expires = tmp; }
						...
						if (switch_true(val)) { sofia_set_pflag(profile, PFLAG_TRACK_CALLS); }
						...
						if (zstr(val)) {
							...
							if (!strncasecmp(ifname, "auto/", 5)) {
								ifname += 5;
								family = AF_UNSPEC;
							}
							if (!strncasecmp(ifname, "ipv4/", 5)) {
								ifname += 5;
								family = AF_INET;
							}
							if (!strncasecmp(ifname, "ipv6/", 5)) {
								ifname += 5;
								family = AF_INET6;
							}
							if (switch_find_interface_ip(buf, sizeof(buf), NULL, ifname, family) ==
								SWITCH_STATUS_SUCCESS) {
								ip = buf;
								switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
						...
						if (strchr(ip, ':')) {
							if (profile->rtpip_index6 < MAX_RTPIP) {
								...
								switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
												  "Max IPs configured for profile %s.\n", profile->name);
							}
						} else {
						...
						if (!zstr(val)) {
							...
							if (!strcasecmp(val, "0.0.0.0")) {
								...
								if (sofia_glue_ext_address_lookup(profile, &myip, &profile->extsipport, val,
						...
						if (v_session_timeout >= 0) { profile->session_timeout = v_session_timeout; }
					} else if (!strcasecmp(var, "max-proceeding") && !zstr(val)) {
						int v_max_proceeding = atoi(val);
						if (v_max_proceeding >= 0) { profile->max_proceeding = v_max_proceeding; }
					...
					if (!strcasecmp(var, "multiple-registrations")) {
						...
						if (profile->minimum_session_expires < 90) { profile->minimum_session_expires = 90; }
						...
						if (!strcasecmp(val, "none")) {
							...
							if (!profile->extsipip && profile->sipip &&
								switch_check_network_list_ip(profile->sipip, val)) {
								switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
						...
						if ((p = strrchr(profile->ws_ip, ':'))) {
							*p++ = '\0';
							...
							if ((tmp = atol(p)) && tmp > 0) { profile->ws_port = (switch_port_t)tmp; }
						}
						...
						if ((p = strrchr(profile->wss_ip, ':'))) {
							*p++ = '\0';
							...
							if ((tmp = atol(p)) && tmp > 0) { profile->wss_port = (switch_port_t)tmp; }
						}
						...
						if (switch_true(val)) {
							...
							if (profile->tls_bind_params) {
								if (!switch_stristr("transport=tls", profile->tls_bind_params)) {
									profile->tls_bind_params = switch_core_sprintf(profile->pool, "%s;transport=tls",
																				   profile->tls_bind_params);
								}
							} else {
						...
						while (ps && *pe) {
							...
							n = (int)(pe - ps);
							if (n == 5 && !strncasecmp(ps, "sslv2", n)) profile->tls_version |= SOFIA_TLS_VERSION_SSLv2;
							if (n == 5 && !strncasecmp(ps, "sslv3", n)) profile->tls_version |= SOFIA_TLS_VERSION_SSLv3;
							if (n == 6 && !strncasecmp(ps, "sslv23", n))
								profile->tls_version |= SOFIA_TLS_VERSION_SSLv2 | SOFIA_TLS_VERSION_SSLv3;
							if (n == 5 && !strncasecmp(ps, "tlsv1", n)) profile->tls_version |= SOFIA_TLS_VERSION_TLSv1;
						...
						char *algs_arr[100] = {0};
						uint8_t algs =
							switch_separate_string(val, ',', algs_arr, (sizeof(algs_arr) / sizeof(algs_arr[0])));
				...
				if (!max_recv_requests_per_second_initialized) { profile->max_recv_requests_per_second = 1000; }
				...
				if (sofia_test_flag(profile, TFLAG_INB_NOMEDIA) && !sofia_test_flag(profile, TFLAG_LATE_NEGOTIATION)) {
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
									  "inbound-bypass-media  implictly enables inbound-late-negotiation\n");
					sofia_set_flag(profile, TFLAG_LATE_NEGOTIATION);
				}

				if (sofia_test_pflag(profile, PFLAG_SEND_DISPLAY_UPDATE) &&
					!sofia_test_pflag(profile, PFLAG_ALLOW_UPDATE)) {
					switch_log_printf(
						SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
						"send-display-update=true is set, but we can't comply because allow-update=false\n");
					sofia_clear_pflag(profile, PFLAG_SEND_DISPLAY_UPDATE);
				}
				if (sofia_test_pflag(profile, PFLAG_PROXY_HOLD)) {
					if (profile->media_options & MEDIA_OPT_MEDIA_ON_HOLD) {
						...
						switch_log_printf(
							SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
							"proxy-hold=true is set, incompatible with media-option=resume-media-on-hold\n");
					}
					if (profile->media_options & MEDIA_OPT_BYPASS_AFTER_HOLD) {
						profile->media_options &= ~MEDIA_OPT_BYPASS_AFTER_HOLD;
						switch_log_printf(
							SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
							"proxy-hold=true is set, incompatible with media-option=bypass-media-after-hold\n");
					}
				}
				...
				if (!profile->sipip) { profile->sipip = switch_core_strdup(profile->pool, mod_sofia_globals.guess_ip); }

				if (!profile->rtpip[0] && !profile->rtpip6[0]) {
					profile->rtpip[profile->rtpip_index++] =
						switch_core_strdup(profile->pool, mod_sofia_globals.guess_ip);
				}

				if (switch_nat_get_type()) {
					char *ip = switch_core_get_variable_dup("nat_public_addr");
					if (ip && !strchr(profile->sipip, ':')) {
						if (!profile->extrtpip) { profile->extrtpip = switch_core_strdup(profile->pool, ip); }
						if (!profile->extsipip) { profile->extsipip = switch_core_strdup(profile->pool, ip); }
						sofia_set_pflag(profile, PFLAG_AUTO_NAT);
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,
										  "NAT detected setting external ip to %s\n", ip);
					}
					switch_safe_free(ip);
				}
				...
				if (!profile->sdp_username) { profile->sdp_username = switch_core_strdup(profile->pool, "FreeSWITCH"); }
				...
				if (!profile->dialplan) { profile->dialplan = switch_core_strdup(profile->pool, "XML"); }

				if (!profile->context) { profile->context = switch_core_strdup(profile->pool, "default"); }

				if (!profile->sipdomain) { profile->sipdomain = switch_core_strdup(profile->pool, profile->sipip); }

				if (profile->pres_type == PRES_TYPE_PNP) {
					if (!profile->pnp_prov_url) {
						profile->pnp_prov_url =
							switch_core_sprintf(profile->pool, "http://%s/provision/", mod_sofia_globals.guess_ip);
					}
					...
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
									  "we're configured to provision to [%s] on profile [%s]\n", profile->pnp_prov_url,
									  profile->pnp_notify_profile);
				}
			...
			if (profile) {
				if (profile_already_started) {
					...
					if ((domains_tag = switch_xml_child(xprofile, "domains"))) {
						...
						for (domain_tag = switch_xml_child(domains_tag, "domain"); domain_tag;
							 domain_tag = domain_tag->next) {
							switch_xml_t droot, x_domain_tag;
							const char *dname = switch_xml_attr_soft(domain_tag, "name");
							const char *parse = switch_xml_attr_soft(domain_tag, "parse");
							const char *alias = switch_xml_attr_soft(domain_tag, "alias");
							...
							if (!zstr(dname)) {
								if (!strcasecmp(dname, "all")) {
									switch_xml_t xml_root, x_domains;
									if (switch_xml_locate("directory", NULL, NULL, NULL, &xml_root, &x_domains,
														  xml_params, SWITCH_FALSE) == SWITCH_STATUS_SUCCESS) {
										for (x_domain_tag = switch_xml_child(x_domains, "domain"); x_domain_tag;
											 x_domain_tag = x_domain_tag->next) {
											dname = switch_xml_attr_soft(x_domain_tag, "name");
											parse_domain_tag(profile, x_domain_tag, dname, parse, alias);
					...
					if ((aliases_tag = switch_xml_child(xprofile, "aliases"))) {
						for (alias_tag = switch_xml_child(aliases_tag, "alias"); alias_tag;
							 alias_tag = alias_tag->next) {
							char *aname = (char *)switch_xml_attr_soft(alias_tag, "name");
							if (!zstr(aname)) {

								if (sofia_glue_add_profile(switch_core_strdup(profile->pool, aname), profile) ==
									SWITCH_STATUS_SUCCESS) {
									switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,
					...
					if (profile->sipip) {
						...
						if ((switch_event_create_subclass(&s_event, SWITCH_EVENT_CUSTOM, MY_EVENT_PROFILE_START) ==
							 SWITCH_STATUS_SUCCESS)) {
							switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "module_name", "mod_sofia");
							switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "profile_name", profile->name);
							switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "profile_uri", profile->url);
	...
	if (profile_already_started) { sofia_glue_release_profile(profile_already_started); }
									   ...
									   sip_t const *sip, sofia_dispatch_event_t *de, tagi_t tags[])
{
	...
	if (gateway) {
		if (status >= 200 && status < 600 && status != 408 && status != 503) {
			if (gateway->state == REG_STATE_FAILED) { gateway->state = REG_STATE_UNREGED; }

			if (gateway->ping_count < 0) { gateway->ping_count = 0; }
			...
			if (gateway->ping_count < gateway->ping_max) {
				...
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
								  ...
								  gateway->ping_min, gateway->ping_count, gateway->ping_max,
								  sofia_gateway_status_name(gateway->status));
			}
			...
			if (gateway->ping_count > 0) { gateway->ping_count--; }
		...
		sofia_reg_release_gateway(gateway);
		gateway->pinging = 0;
	} else if (sip && sip->sip_to && sip->sip_call_id && sip->sip_call_id->i_id &&
			   strchr(sip->sip_call_id->i_id, '_')) {
		...
		sql = switch_mprintf("select ping_status, ping_count, contact from sip_registrations where sip_user='%q' and "
							 "sip_host='%q' and call_id='%q'",
							 sip->sip_to->a_url->url_user, sip->sip_to->a_url->url_host, call_id);
		sofia_glue_execute_sql_callback(profile, profile->ireg_mutex, sql, sofia_sip_user_status_callback,
										&sip_user_status);
		...
		if (status != 200 && status != 486) {
			...
			if (sip_user_status.count >= 0) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
								  "Ping to sip user '%s@%s' failed with code %d - count %d, state %s\n",
								  sip->sip_to->a_url->url_user, sip->sip_to->a_url->url_host, status,
								  sip_user_status.count, sip_user_status.status);
				sql = switch_mprintf("update sip_registrations set ping_count=%d, ping_time=%d where sip_user='%q' and "
									 "sip_host='%q' and call_id='%q'",
									 sip_user_status.count, ping_time, sip->sip_to->a_url->url_user,
									 sip->sip_to->a_url->url_host, call_id);
				sofia_glue_execute_sql(profile, &sql, SWITCH_TRUE);
				switch_safe_free(sql);
			}
			if (sip_user_status.count < sip_user_ping_min) {
				if (strcmp(sip_user_status.status, "Unreachable")) {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
									  "Sip user '%s@%s' is now Unreachable\n", sip->sip_to->a_url->url_user,
									  sip->sip_to->a_url->url_host);
					sql =
						switch_mprintf("update sip_registrations set ping_status='Unreachable', ping_time=%d where "
									   "sip_user='%q' and sip_host='%q' and call_id='%q'",
									   ping_time, sip->sip_to->a_url->url_user, sip->sip_to->a_url->url_host, call_id);
					sofia_glue_execute_sql(profile, &sql, SWITCH_TRUE);
					switch_safe_free(sql);
					sofia_reg_fire_custom_sip_user_state_event(
						profile, sip_user, sip_user_status.contact, sip->sip_to->a_url->url_user,
						sip->sip_to->a_url->url_host, call_id, SOFIA_REG_REACHABLE, status, phrase);

					if (sofia_test_pflag(profile, PFLAG_UNREG_OPTIONS_FAIL)) {
						time_t now = switch_epoch_time_now(NULL);
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
										  "Expire sip user '%s@%s' due to options failure\n",
										  sip->sip_to->a_url->url_user, sip->sip_to->a_url->url_host);

						sql = switch_mprintf("update sip_registrations set expires=%ld, ping_time=%d where "
											 "sip_user='%q' and sip_host='%q' and call_id='%q'",
											 (long)now, ping_time, sip->sip_to->a_url->url_user,
											 sip->sip_to->a_url->url_host, call_id);
						sofia_glue_execute_sql(profile, &sql, SWITCH_TRUE);
						switch_safe_free(sql);
					}
			...
			if (sip_user_status.count <= sip_user_ping_max) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
								  "Ping to sip user '%s@%s' succeeded with code %d - count %d, state %s\n",
								  sip->sip_to->a_url->url_user, sip->sip_to->a_url->url_host, status,
								  sip_user_status.count, sip_user_status.status);
				sql = switch_mprintf("update sip_registrations set ping_count=%d, ping_time=%d where sip_user='%q' and "
									 "sip_host='%q' and call_id='%q'",
			...
			if (sip_user_status.count >= sip_user_ping_min) {
				if (strcmp(sip_user_status.status, "Reachable")) {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
									  "Sip user '%s@%s' is now Reachable\n", sip->sip_to->a_url->url_user,
									  sip->sip_to->a_url->url_host);
					sql = switch_mprintf("update sip_registrations set ping_status='Reachable' where sip_user='%q' and "
										 "sip_host='%q' and call_id='%q'",
										 sip->sip_to->a_url->url_user, sip->sip_to->a_url->url_host, call_id);
					sofia_glue_execute_sql(profile, &sql, SWITCH_TRUE);
					switch_safe_free(sql);
									  ...
									  sip_t const *sip, sofia_dispatch_event_t *de, tagi_t tags[])
{
	...
	if (sip && session) {
		...
		if (de && de->data && de->data->e_msg) {
			...
			switch_channel_set_variable_printf(channel, "sip_local_network_addr", "%s",
		...
		if (status < 200) {
			...
			if (sofia_test_flag(tech_pvt, TFLAG_SKIP_EARLY)) { return; }
		}
		...
		if (status > 299) { switch_channel_set_variable(channel, "sip_hangup_disposition", "recv_refuse"); }
		...
		if (status >= 400 && sip->sip_reason && sip->sip_reason->re_protocol &&
			(!strcasecmp(sip->sip_reason->re_protocol, "Q.850") ||
			 !strcasecmp(sip->sip_reason->re_protocol, "FreeSWITCH") ||
			 !strcasecmp(sip->sip_reason->re_protocol, profile->sdp_username)) &&
			sip->sip_reason->re_cause) {
		...
		if (sip->sip_payload && sip->sip_payload->pl_data) {
			...
			if (!zstr(tech_pvt->mparams.prev_sdp_response) &&
				!strcmp(tech_pvt->mparams.prev_sdp_response, sip->sip_payload->pl_data)) {
		...
		if (sofia_test_pflag(profile, PFLAG_MANAGE_SHARED_APPEARANCE)) {
			if (channel && sip->sip_call_info) {
				...
				if (status != 200) { state = "progressing"; }

				if (sip->sip_from && sip->sip_from->a_url->url_user && sip->sip_from->a_url->url_host && sip->sip_to &&
					sip->sip_to->a_url->url_user && sip->sip_to->a_url->url_host) {
					sql = switch_mprintf("select 'appearance-index=1' from sip_subscriptions where expires > -1 and "
										 "hostname='%q' and event='call-info' and "
										 "sub_to_user='%q' and sub_to_host='%q'",
										 mod_sofia_globals.hostname, sip->sip_to->a_url->url_user,
										 sip->sip_from->a_url->url_host);
					...
					if (!zstr(buf)) {
						sql = switch_mprintf("update sip_dialogs set call_info='%q',call_info_state='%q' "
											 "where uuid='%q'",
											 buf, state, switch_core_session_get_uuid(session));
						...
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
										  "Auto-Fixing Broken SLA [<sip:%s>;%s]\n", sip->sip_from->a_url->url_host,
										  buf);
						switch_channel_set_variable_printf(channel, "presence_call_info_full", "<sip:%s>;%s",
		...
		if (channel && (status == 300 || status == 301 || status == 302 || status == 305) &&
			...
			if ((v = switch_channel_get_variable(channel, "outbound_redirect_fatal")) && switch_true(v)) {
				...
				for (p_contact = sip->sip_contact; p_contact; p_contact = p_contact->m_next) {
					...
					if (i == 0) { switch_channel_set_variable(channel, "sip_redirected_to", full_contact); }
			...
			if (!p_contact) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
								  "Missing contact header in redirect request\n");
				goto end;
			}
			...
			if ((br = switch_channel_get_partner_uuid(channel))) {
				...
				if ((a_session = switch_core_session_locate(br)) &&
					...
					if (switch_true(sip_redirect_fork)) { *separator = ','; }
					...
					for (p_contact = sip->sip_contact; p_contact; p_contact = p_contact->m_next) {
						...
						switch_channel_set_variable_printf(channel, var_name, "sofia/%s/%s", sip_redirect_profile,
														   invite_contact);
						switch_channel_set_variable_printf(a_channel, var_name, "sofia/%s/%s", sip_redirect_profile,
														   invite_contact);
						stream.write_function(&stream, "%ssofia/%s/%s", i ? separator : "", sip_redirect_profile,
											  invite_contact);
						free(invite_contact);
						i++;
					}
					...
					if (sofia_test_pflag(profile, PFLAG_MANUAL_REDIRECT)) {
						if (!switch_channel_get_variable(channel, "outbound_redirect_info")) {
							...
							switch_ivr_session_transfer(a_session, p_contact->m_url->url_user, sip_redirect_dialplan,
						...
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
										  "Received redirect with invalid URI\n");
						...
						switch_channel_hangup(channel, SWITCH_CAUSE_DESTINATION_OUT_OF_ORDER);
							   ...
							   (switch_xml_locate_domain(p_contact->m_url->url_host, NULL, &root, &domain) ==
								SWITCH_STATUS_SUCCESS)) {
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
										  "Redirect: Transfering to %s\n", p_contact->m_url->url_user);
			...
			if (sofia_test_flag(tech_pvt, TFLAG_SENT_UPDATE)) {
				...
				if ((uuid = switch_channel_get_partner_uuid(channel)) &&
					...
					if (sip->sip_payload && sip->sip_payload->pl_data && sip->sip_content_type &&
						...
						tech_pvt->mparams.remote_sdp_str =
							switch_core_session_strdup(tech_pvt->session, sip->sip_payload->pl_data);
					...
					} else if (status == 200 && switch_channel_test_flag(channel, CF_T38_PASSTHRU) && has_t38 &&
							   sip->sip_payload && sip->sip_payload->pl_data) {
						switch_t38_options_t *t38_options =
							switch_core_media_extract_t38_options(session, sip->sip_payload->pl_data);
		...
		if ((status == 180 || status == 183 || status == 200)) {
			...
			if (sip->sip_to) { to = sip->sip_to->a_url; }
			if (sip->sip_from) { from = sip->sip_from->a_url; }
			if (sip->sip_contact) { contact = sip->sip_contact->m_url; }

			if (status == 200) { astate = "confirmed"; }

			if ((!switch_channel_test_flag(channel, CF_EARLY_MEDIA) &&
				 !switch_channel_test_flag(channel, CF_ANSWERED) &&
				 !switch_channel_test_flag(channel, CF_RING_READY)) ||
				switch_channel_test_flag(channel, CF_RECOVERING)) {
				const char *from_user = "", *from_host = "", *to_user = "", *to_host = "", *contact_user = "",
						   *contact_host = "";
				...
				if (sip->sip_user_agent) { user_agent = switch_str_nil(sip->sip_user_agent->g_string); }

				if (sip->sip_call_id) { call_id = switch_str_nil(sip->sip_call_id->i_id); }
				...
				if (profile->pres_type) {
					...
					if (call_info && (p = strchr(call_info, ';'))) { p++; }
					...
					sql = switch_mprintf("insert into sip_dialogs "
										 ...
										 "sip_from_tag,contact_user,"
										 "contact_host,state,direction,user_agent,profile_name,hostname,contact,"
										 "presence_id,presence_data,"
										 "call_info,rcd,call_info_state) "
										 "values('%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q','%q',"
										 "'%q','%q','%q','%q','%q',%ld,'')",
										 call_id, switch_core_session_get_uuid(session), to_user, to_host, to_tag,
										 from_user, from_host, from_tag, contact_user, contact_host, astate, "outbound",
										 user_agent, profile->name, mod_sofia_globals.hostname,
										 switch_str_nil(full_contact), switch_str_nil(presence_id),
										 switch_str_nil(presence_data), switch_str_nil(p), (long)now);
					switch_assert(sql);

					sofia_glue_execute_sql_now(profile, &sql, SWITCH_TRUE);

					if (full_contact) { su_free(nua_handle_home(tech_pvt->nh), full_contact); }
				}
	...
	if (call_info) { su_free(nua_handle_home(nh), call_info); }
...
static void mark_transfer_record(switch_core_session_t *session, const char *br_a, const char *br_b)
{
	...
	if ((br_b_session = switch_core_session_locate(br_b))) {
		...
		cp->transfer_source = switch_core_sprintf(
			cp->pool, "%ld:%s:att_xfer:%s@%s/%s@%s", (long)switch_epoch_time_now(NULL), cp->uuid_str,
			switch_channel_get_variable(channel, uvar1), switch_channel_get_variable(channel, dvar1),
			switch_channel_get_variable(br_b_channel, uvar2), switch_channel_get_variable(br_b_channel, dvar2));

		switch_channel_add_variable_var_check(br_b_channel, SWITCH_TRANSFER_HISTORY_VARIABLE, cp->transfer_source,
	...
	if ((br_a_session = switch_core_session_locate(br_a))) {
		...
		cp->transfer_source = switch_core_sprintf(
			cp->pool, "%ld:%s:att_xfer:%s@%s/%s@%s", (long)switch_epoch_time_now(NULL), cp->uuid_str,
			switch_channel_get_variable(channel, uvar1), switch_channel_get_variable(channel, dvar1),
			switch_channel_get_variable(br_a_channel, uvar2), switch_channel_get_variable(br_a_channel, dvar2));

		switch_channel_add_variable_var_check(br_a_channel, SWITCH_TRANSFER_HISTORY_VARIABLE, cp->transfer_source,
											  SWITCH_FALSE, SWITCH_STACK_PUSH);
									 ...
									 sip_t const *sip, sofia_dispatch_event_t *de, tagi_t tags[])
{
	...
	tl_gets(tags, NUTAG_CALLSTATE_REF(ss_state), NUTAG_OFFER_RECV_REF(offer_recv), NUTAG_ANSWER_RECV_REF(answer_recv),
			NUTAG_OFFER_SENT_REF(offer_sent), NUTAG_ANSWER_SENT_REF(answer_sent), SIPTAG_REPLACES_STR_REF(replaces_str),
	...
	if (session) {
		...
		if (!tech_pvt || !tech_pvt->nh) { goto done; }

		if (!r_sdp && (status == 100 || status == 200)) {
			if (ss_state == nua_callstate_completing) {
				if (tech_pvt->mparams.last_sdp_response) { r_sdp = tech_pvt->mparams.last_sdp_response; }
			} else if (ss_state == nua_callstate_received || ss_state == nua_callstate_ready) {
				if (tech_pvt->mparams.last_sdp_str) { r_sdp = tech_pvt->mparams.last_sdp_str; }
			}
		}

		if (tech_pvt->mparams.last_sdp_str) { tech_pvt->mparams.prev_sdp_str = tech_pvt->mparams.last_sdp_str; }
		...
		if (r_sdp &&
			...
			if ((var = switch_channel_get_variable(channel, "bypass_media_sdp_filter"))) {
				if ((patched_sdp = switch_core_media_process_sdp_filter(r_sdp, var, session))) { r_sdp = patched_sdp; }
			}
		}
		...
		if ((switch_channel_test_flag(channel, CF_PROXY_MODE) || switch_channel_test_flag(channel, CF_PROXY_MEDIA)) ||
			...
			if (profile->ndlb & SM_NDLB_ALLOW_BAD_IANANAME && r_sdp &&
				(p = (char *)switch_stristr("g729a/8000", r_sdp))) {
				p += 4;
				*p++ = '/';
				*p++ = '8';
				*p++ = '0';
	...
	if (ss_state == nua_callstate_terminated) {
		...
		if (sofia_private) { sofia_private->destroy_me = 1; }
	}

	if (session) {
		if ((switch_channel_test_flag(channel, CF_ANSWERED) && (status == 180 || status == 183) && !r_sdp) ||
			(ss_state == nua_callstate_ready && status >= 300)) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
		...
		if (r_sdp) {
			...
			if (!(profile->mndlb & SM_NDLB_ALLOW_NONDUP_SDP) ||
				(!zstr(tech_pvt->mparams.remote_sdp_str) && !strcmp(tech_pvt->mparams.remote_sdp_str, r_sdp))) {
				...
				// SWITCH_CALL_DIRECTION_OUTBOUND)) { 	switch_core_media_set_sdp_codec_string(session, r_sdp, status < 200
	...
	if (status == 988) { goto done; }

	if (status == 183 && !r_sdp) {
		if ((channel && switch_true(switch_channel_get_variable(channel, "sip_ignore_183nosdp"))) ||
			sofia_test_pflag(profile, PFLAG_IGNORE_183NOSDP)) {
	...
	if (status == 180 && r_sdp) { status = 183; }
	...
	if (channel && profile->pres_type && ss_state == nua_callstate_ready && status == 200) {
		...
		sql = switch_mprintf("update sip_dialogs set sip_to_tag='%q' "
							 "where uuid='%q' and sip_to_tag = ''",
							 to_tag, switch_core_session_get_uuid(session));
	...
	if (channel && (status == 180 || status == 183) &&
		...
		if ((var = switch_channel_get_variable(channel, "sip_auto_answer")) && switch_true(var) &&
			...
			nua_notify(nh, NUTAG_NEWSUB(1), NUTAG_WITH_THIS_MSG(de->data->e_msg),
					   NUTAG_SUBSTATE(nua_substate_terminated), TAG_IF((full_to), SIPTAG_TO_STR(full_to)),
					   SIPTAG_SUBSCRIPTION_STATE_STR("terminated;reason=noresource"), SIPTAG_EVENT_STR("talk"),
					   TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)), TAG_END());
		}
	}
...
state_process:
		...
		if (r_sdp) {
			if (switch_channel_test_flag(channel, CF_PROXY_MODE)) {
				...
				if (switch_channel_test_flag(channel, CF_PROXY_MEDIA) &&
					switch_channel_direction(tech_pvt->channel) == SWITCH_CALL_DIRECTION_INBOUND) {
				...
				sofia_set_flag(tech_pvt, TFLAG_SDP);
				if (switch_channel_test_flag(channel, CF_PROXY_MEDIA) ||
					switch_channel_test_flag(channel, CF_REINVITE)) {
					if (sofia_media_activate_rtp(tech_pvt) != SWITCH_STATUS_SUCCESS) { goto done; }
				}
				if (switch_core_session_get_partner(session, &other_session) == SWITCH_STATUS_SUCCESS) {
	...
	case nua_callstate_completing: {
		...
		if (switch_channel_test_flag(channel, CF_3P_NOMEDIA_REQUESTED)) {
					...
					if (switch_core_session_compare(session, other_session)) {
						...
						if (sofia_use_soa(other_tech_pvt)) {
							nua_ack(other_tech_pvt->nh,
									...
									SIPTAG_CONTACT_STR(other_tech_pvt->reply_contact), SOATAG_USER_SDP_STR(r_sdp),
									SOATAG_REUSE_REJECTED(1), SOATAG_RTP_SELECT(1),
									SOATAG_AUDIO_AUX("cn telephone-event"),
									TAG_IF(sofia_test_pflag(other_tech_pvt->profile, PFLAG_DISABLE_100REL),
										   NUTAG_INCLUDE_EXTRA_SDP(1)),
									TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)), TAG_END());
						} else {
							nua_ack(other_tech_pvt->nh, NUTAG_MEDIA_ENABLE(0),
									TAG_IF(!zstr(other_tech_pvt->user_via), SIPTAG_VIA_STR(other_tech_pvt->user_via)),
									SIPTAG_CONTACT_STR(other_tech_pvt->reply_contact),
									TAG_IF(r_sdp, SIPTAG_CONTENT_TYPE_STR("application/sdp")),
									TAG_IF(r_sdp, SIPTAG_PAYLOAD_STR(r_sdp)), SOATAG_AUDIO_AUX("cn telephone-event"),
									TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)), TAG_END());
						}

						nua_ack(tech_pvt->nh, TAG_IF(!zstr(tech_pvt->user_via), SIPTAG_VIA_STR(tech_pvt->user_via)),
			...
			if (tech_pvt->mparams.num_codecs) { match = sofia_media_negotiate_sdp(session, r_sdp, SDP_OFFER); }
			...
			if (!match) {
				...
				if (sofia_use_soa(tech_pvt)) {
					nua_ack(
						tech_pvt->nh, TAG_IF(!zstr(tech_pvt->user_via), SIPTAG_VIA_STR(tech_pvt->user_via)),
						SIPTAG_CONTACT_STR(tech_pvt->reply_contact),
						SOATAG_USER_SDP_STR(tech_pvt->mparams.local_sdp_str), SOATAG_REUSE_REJECTED(1),
						SOATAG_RTP_SELECT(1), SOATAG_AUDIO_AUX("cn telephone-event"),
						TAG_IF(sofia_test_pflag(tech_pvt->profile, PFLAG_DISABLE_100REL), NUTAG_INCLUDE_EXTRA_SDP(1)),
						TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)), TAG_END());
				} else {
					nua_ack(
						tech_pvt->nh, NUTAG_MEDIA_ENABLE(0),
						TAG_IF(!zstr(tech_pvt->user_via), SIPTAG_VIA_STR(tech_pvt->user_via)),
						SIPTAG_CONTACT_STR(tech_pvt->reply_contact),
		...
		if (!sofia_test_flag(tech_pvt, TFLAG_SDP)) {
			...
			if (r_sdp && !sofia_test_flag(tech_pvt, TFLAG_SDP)) {
				if (switch_channel_test_flag(channel, CF_PROXY_MODE)) {
					...
					sofia_set_flag_locked(tech_pvt, TFLAG_READY);
					if (switch_channel_get_state(channel) == CS_NEW) { switch_channel_set_state(channel, CS_INIT); }
					sofia_set_flag(tech_pvt, TFLAG_SDP);
					goto done;
					...
					if (!match) {
						...
						if (replaces_str) {
							...
							if ((replaces = sip_replaces_make(home, replaces_str)) &&
								...
								while (switch_channel_get_state(channel) < CS_EXECUTE) { switch_yield(10000); }
								...
								if ((b_private = nua_handle_magic(bnh))) {
									...
									if (br_b) {
										...
										if (switch_true(
												switch_channel_get_variable(channel, "recording_follow_transfer")) &&
				...
				if (sofia_test_pflag(profile, PFLAG_3PCC)) {
					if (switch_channel_test_flag(channel, CF_PROXY_MODE) ||
						...
						switch_log_printf(
							SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
						...
						if (sofia_use_soa(tech_pvt)) {
							nua_respond(
								tech_pvt->nh, SIP_200_OK, SIPTAG_CONTACT_STR(tech_pvt->reply_contact),
								SOATAG_USER_SDP_STR(tech_pvt->mparams.local_sdp_str), SOATAG_REUSE_REJECTED(1),
								SOATAG_AUDIO_AUX("cn telephone-event"),
							...
							nua_respond(
								tech_pvt->nh, SIP_200_OK, NUTAG_MEDIA_ENABLE(0),
								SIPTAG_CONTACT_STR(tech_pvt->reply_contact), SIPTAG_CONTENT_TYPE_STR("application/sdp"),
					...
					// so can be made to work with bypass media as we have time to find out what the other end thinks
					// codec offer should be...
					switch_channel_set_variable(channel, SWITCH_ENDPOINT_DISPOSITION_VARIABLE, "RECEIVED_NOSDP");
					sofia_set_flag_locked(tech_pvt, TFLAG_3PCC);
					// switch_core_media_choose_port(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO, 0);
					...
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
									  "No SDP in INVITE and 3pcc not enabled, hanging up.\n");
					switch_channel_set_variable(channel, SWITCH_ENDPOINT_DISPOSITION_VARIABLE, "3PCC DISABLED");
					switch_channel_hangup(channel, SWITCH_CAUSE_MANDATORY_IE_MISSING);
				}
				goto done;
			...
			if ((switch_channel_test_flag(channel, CF_PROXY_MODE) ||
				 switch_channel_test_flag(channel, CF_PROXY_MEDIA)) &&
				sofia_test_pflag(profile, PFLAG_3PCC_PROXY)) {
				sofia_set_flag_locked(tech_pvt, TFLAG_3PCC);
			...
			if (switch_channel_var_true(channel, "sip_unhold_nosdp")) {
				switch_core_media_gen_local_sdp(session, SDP_ANSWER, NULL, 0, "sendrecv",
												zstr(tech_pvt->mparams.local_sdp_str) ||
													!switch_channel_test_flag(channel, CF_PROXY_MODE));
			} else {
			...
			if (zstr(tech_pvt->mparams.local_sdp_str)) {
				...
				if (sofia_use_soa(tech_pvt)) {
					...
					nua_respond(tech_pvt->nh, SIP_200_OK, NUTAG_MEDIA_ENABLE(0),
								SIPTAG_CONTACT_STR(tech_pvt->reply_contact), SIPTAG_CONTENT_TYPE_STR("application/sdp"),
								SIPTAG_PAYLOAD_STR(tech_pvt->mparams.local_sdp_str),
								TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)), TAG_END());
				}
	...
	case nua_callstate_early:
		if (answer_recv) {
			...
			if (match) {
				if (switch_core_media_choose_port(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO, 0) !=
					SWITCH_STATUS_SUCCESS) {
					goto done;
				}
	...
	case nua_callstate_completed:
		if (r_sdp) {
			...
			if (switch_stristr("m=image", r_sdp)) { is_t38 = 1; }
			...
			if (switch_channel_test_flag(channel, CF_PROXY_MODE) || switch_channel_test_flag(channel, CF_PROXY_MEDIA)) {
				...
				if (switch_core_session_get_partner(session, &other_session) == SWITCH_STATUS_SUCCESS) {
					...
					int media_on_hold = switch_true(
						switch_channel_get_variable_dup(channel, "bypass_media_resume_on_hold", SWITCH_FALSE, -1));
					...
					if (switch_channel_test_flag(channel, CF_PROXY_MODE) && !is_t38 &&
						...
						if (switch_stristr("sendonly", r_sdp) || switch_stristr("0.0.0.0", r_sdp) ||
							...
							switch_core_media_clear_rtp_flag(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO,
				...
				if (sofia_test_flag(tech_pvt, TFLAG_SIP_HOLD)) {
					hold_related = 2;
				} else if (switch_stristr("sendonly", r_sdp) || switch_stristr("0.0.0.0", r_sdp) ||
						   switch_stristr("inactive", r_sdp)) {
					hold_related = 1;
				}

				if (!is_t38 && hold_related &&
						...
						if (sofia_test_pflag(profile, PFLAG_PROXY_HOLD)) {
							...
							if (hold_related == 1) {
								...
								if (sofia_test_pflag(profile, PFLAG_MANAGE_SHARED_APPEARANCE)) {
									...
									if (info) {
										if (switch_stristr("private", info)) { hold_msg = "hold-private"; }
									}
								}
								sofia_set_flag_locked(tech_pvt, TFLAG_SIP_HOLD);
								switch_channel_set_flag(channel, CF_LEG_HOLDING);
				...
				if (match) {
					...
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
									  "Reinvite resulted in codec negotiation failure.\n");
					is_ok = 0;
				}
			}
			...
			if (is_ok) {
				...
				if (switch_event_create_subclass(&s_event, SWITCH_EVENT_CUSTOM, MY_EVENT_REINVITE) ==
					SWITCH_STATUS_SUCCESS) {
					switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "Unique-ID",
												   switch_core_session_get_uuid(session));
					switch_event_fire(&s_event);
				}
			} else {
	...
	case nua_callstate_ready:
		if (!switch_channel_test_flag(channel, CF_PROXY_MODE) && !switch_channel_test_flag(channel, CF_PROXY_MEDIA) &&
			r_sdp && (!is_dup_sdp || sofia_test_flag(tech_pvt, TFLAG_NEW_SDP)) &&
			switch_core_media_ready(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO) &&
			!sofia_test_flag(tech_pvt, TFLAG_NOSDP_REINVITE)) {
			/* sdp changed since 18X w sdp, we're supposed to ignore it but we, of course, were pressured into
			 * supporting it */
			uint8_t match = 0;
			...
			if (tech_pvt->mparams.num_codecs) { match = sofia_media_negotiate_sdp(session, r_sdp, SDP_ANSWER); }
			if (match) {
				if (switch_core_media_choose_port(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO, 0) !=
					SWITCH_STATUS_SUCCESS) {
					goto done;
		...
		if (channel) { switch_channel_clear_flag(channel, CF_REQ_MEDIA); }
					...
					if (match) {
						...
						if (switch_core_media_choose_port(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO, 0) ==
							...
							if (sofia_test_flag(tech_pvt, TFLAG_3PCC)) {
								/* Check if we are in 3PCC proxy mode, if so then set the flag to indicate we received
								 * the ack */
	...
	case nua_callstate_terminated:
		...
		if (ss_state == nua_callstate_terminated) {
			if (tech_pvt->sofia_private) { tech_pvt->sofia_private = NULL; }
...
void *SWITCH_THREAD_FUNC nightmare_xfer_thread_run(switch_thread_t *thread, void *obj)
{
	...
	if ((a_session = switch_core_session_locate(nhelper->bridge_to_uuid))) {
		...
		if ((session = switch_core_session_locate(nhelper->reply_uuid))) {
			...
			if ((status = switch_ivr_originate(NULL, &tsession, &cause, nhelper->exten_with_params, timeout, NULL, NULL,
											   NULL, switch_channel_get_caller_profile(channel_a), nhelper->vars,
...
static switch_status_t xfer_hanguphook(switch_core_session_t *session)
{
	...
	if (state == CS_HANGUP) {
		...
		if (uuid && (ksession = switch_core_session_force_locate(uuid))) {
			...
			if (switch_channel_up(kchannel)) { switch_channel_hangup(kchannel, SWITCH_CAUSE_NORMAL_CLEARING); }
...
void sofia_handle_sip_i_refer(nua_t *nua, sofia_profile_t *profile, nua_handle_t *nh, switch_core_session_t *session,
							  sip_t const *sip, sofia_dispatch_event_t *de, tagi_t tags[])
{
	/* Incoming refer */
	...
	if ((refer_to = sip->sip_refer_to)) { full_ref_to = sip_header_as_string(home, (void *)sip->sip_refer_to); }
	...
	if (sip->sip_referred_by) { full_ref_by = sip_header_as_string(home, (void *)sip->sip_referred_by); }

	if (refer_to) {
		char *rep = NULL;

		if (sofia_test_pflag(profile, PFLAG_FULL_ID)) {
			exten = switch_core_session_sprintf(session, "%s@%s", (char *)refer_to->r_url->url_user,
		...
		if (rep) {
			...
			if (bnh && !sofia_test_pflag(profile, PFLAG_MAKE_EVERY_TRANSFER_A_NIGHTMARE)) {
				...
				if ((b_private = nua_handle_magic(bnh))) {
					...
					if (!switch_ivr_uuid_exists(br_a)) { br_a = NULL; }

					if (!switch_ivr_uuid_exists(br_b)) { br_b = NULL; }

					if (channel_a && switch_true(switch_channel_get_variable(channel_a, "deny_refer_requests"))) {
						deny_refer_requests = 1;
					}

					if (!deny_refer_requests && channel_b &&
						switch_true(switch_channel_get_variable(channel_b, "deny_refer_requests"))) {
						deny_refer_requests = 1;
					}
					...
					if (!deny_refer_requests && br_a) {
						...
						if ((a_session = switch_core_session_locate(br_a))) {
							...
							if (a_channel &&
								switch_true(switch_channel_get_variable(a_channel, "deny_refer_requests"))) {
								deny_refer_requests = 1;
							}
							switch_core_session_rwunlock(a_session);
						}
					...
					if (!deny_refer_requests && br_b) {
						...
						if ((b_session = switch_core_session_locate(br_b))) {
							...
							if (b_channel &&
								switch_true(switch_channel_get_variable(b_channel, "deny_refer_requests"))) {
								deny_refer_requests = 1;
							}
					...
					if (deny_refer_requests) {
						switch_log_printf(
							SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_NOTICE,
							"Denying Attended Transfer, variable [deny_refer_requests] was set to true\n");

						nua_notify(tech_pvt->nh, NUTAG_NEWSUB(1),
						...
						if ((a_session = switch_core_session_locate(br_a))) {
							...
							switch_channel_set_variable(a_channel, "att_xfer_kill_uuid",
														switch_core_session_get_uuid(b_session));
							switch_channel_set_variable(a_channel, "att_xfer_destination_number",
														prof->destination_number);
							...
							if ((tmp = switch_channel_get_hold_music(a_channel))) { moh = tmp; }

							if (!zstr(moh) && !strcasecmp(moh, "silence")) { moh = NULL; }

							if (sofia_test_pflag(profile, PFLAG_FIRE_TRANFER_EVENTS)) {
								if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, MY_EVENT_REPLACED) ==
									SWITCH_STATUS_SUCCESS) {
									switch_channel_event_set_data(channel_b, event);
									switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "att_xfer_replaced_by",
																   br_a);
									switch_event_fire(&event);
								}

								if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, MY_EVENT_TRANSFEROR) ==
									SWITCH_STATUS_SUCCESS) {
									switch_channel_event_set_data(channel_a, event);
									switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
																   "att_xfer_original_call_id", br_a);
									switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
																   "att_xfer_destination_call_id", br_b);
									switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
																   "att_xfer_destination_peer_uuid",
																   switch_core_session_get_uuid(b_session));
									switch_event_fire(&event);
								}

								if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, MY_EVENT_TRANSFEREE) ==
									SWITCH_STATUS_SUCCESS) {
									switch_channel_event_set_data(a_channel, event);
									switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
																   "att_xfer_replaced_call_id",
																   switch_core_session_get_uuid(b_session));
									switch_event_fire(&event);
								}
							}
							...
							if (switch_true(switch_channel_get_variable(channel_b, "recording_follow_transfer")) &&
								(tmpsess = switch_core_session_locate(br_a))) {
								switch_log_printf(
									SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_NOTICE,
							...
							nua_notify(
								tech_pvt->nh, NUTAG_NEWSUB(1), SIPTAG_CONTENT_TYPE_STR("message/sipfrag;version=2.0"),
								NUTAG_SUBSTATE(nua_substate_terminated),
								SIPTAG_SUBSCRIPTION_STATE_STR("terminated;reason=noresource"),
							...
							if (b_tech_pvt && !sofia_test_flag(b_tech_pvt, TFLAG_BYE)) {
								...
								if (!val || switch_true(val)) {
									q850 = switch_core_session_sprintf(a_session,
																	   "Q.850;cause=16;text=\"normal_clearing\"");
								}
								nua_bye(b_tech_pvt->nh, SIPTAG_CONTACT(SIP_NONE),
						...
						if (switch_true(switch_channel_get_variable(channel_a, "recording_follow_transfer")) &&
							(tmp = switch_core_session_locate(br_a))) {
							switch_channel_set_variable(switch_core_session_get_channel(tmp), "transfer_disposition",
														"bridge");
							switch_ivr_transfer_recordings(session, tmp);
							switch_core_session_rwunlock(tmp);
						}
						...
						if (sofia_test_pflag(profile, PFLAG_FIRE_TRANFER_EVENTS)) {
							...
							if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, MY_EVENT_TRANSFEROR) ==
								SWITCH_STATUS_SUCCESS) {
								switch_channel_event_set_data(channel_a, event);
								switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "att_xfer_original_call_id",
															   br_a);
								switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
															   "att_xfer_destination_call_id", br_b);
								switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
						...
						if (!br_a && !br_b) {
							...
							if (br_a && !br_b) {
								...
								private_object_t *h_tech_pvt =
									(private_object_t *)switch_core_session_get_private(b_session);
							...
							if (t_session) {
								...
								if (!zstr(full_ref_by)) {
									switch_channel_set_variable(t_channel, SOFIA_SIP_HEADER_PREFIX "Referred-By",
																full_ref_by);
								}
								...
								if (switch_true(
										switch_channel_get_variable(hup_channel, "recording_follow_transfer"))) {
									switch_ivr_transfer_recordings(hup_session, t_session);
								}

								if (sofia_test_pflag(profile, PFLAG_FIRE_TRANFER_EVENTS)) {
									if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM,
																	 MY_EVENT_TRANSFEROR) == SWITCH_STATUS_SUCCESS) {
										switch_channel_event_set_data(channel_a, event);
										switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
																	   "att_xfer_original_call_id",
																	   switch_core_session_get_uuid(hup_session));
										switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
																	   "att_xfer_destination_call_id",
																	   switch_core_session_get_uuid(t_session));
										switch_event_add_header_string(event, SWITCH_STACK_BOTTOM,
																	   "att_xfer_destination_peer_uuid",
									...
									if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM,
																	 MY_EVENT_TRANSFEREE) == SWITCH_STATUS_SUCCESS) {
								...
								switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
												  "Session to transfer to not found.\n");
								nua_notify(tech_pvt->nh, NUTAG_NEWSUB(1),
										   SIPTAG_CONTENT_TYPE_STR("message/sipfrag;version=2.0"),
										   NUTAG_SUBSTATE(nua_substate_terminated),
										   SIPTAG_SUBSCRIPTION_STATE_STR("terminated;reason=noresource"),
				...
				if (exten && (br_a = switch_channel_get_partner_uuid(channel_a))) {
					...
					if ((a_session = switch_core_session_locate(br_a))) {
						...
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
										  ...
										  switch_core_session_get_uuid(session), rep, exten,
										  (char *)refer_to->r_url->url_host, br_a);

						if (refer_to->r_url->url_port) { port = refer_to->r_url->url_port; }
						...
						if (sofia_test_pflag(profile, PFLAG_CHANNEL_XML_FETCH_ON_NIGHTMARE_TRANSFER)) {
							...
							const char *xml_url = NULL, *use_profile = profile->name, *dial_prefix = NULL,
									   *absolute_dial_string = NULL;
							switch_xml_t params = NULL, param = NULL;

							switch_event_create(&xml_params, SWITCH_EVENT_REQUEST_PARAMS);
							switch_event_add_header_string(xml_params, SWITCH_STACK_BOTTOM, "purpose",
														   "nightmare_xfer");
							switch_event_add_header_string(xml_params, SWITCH_STACK_BOTTOM, "profile", profile->name);
							switch_event_add_header_string(xml_params, SWITCH_STACK_BOTTOM, "refer-to-user",
														   refer_to->r_url->url_user);
							switch_event_add_header_string(xml_params, SWITCH_STACK_BOTTOM, "refer-to-host",
														   refer_to->r_url->url_host);
							switch_event_add_header_string(xml_params, SWITCH_STACK_BOTTOM, "refer-to-params",
														   refer_to->r_url->url_params ? refer_to->r_url->url_params
																					   : "");
							switch_event_add_header_string(xml_params, SWITCH_STACK_BOTTOM, "refer-to-headers",
														   refer_to->r_url->url_headers ? refer_to->r_url->url_headers
																						: "");
							if (replaces) {
								switch_event_add_header_string(xml_params, SWITCH_STACK_BOTTOM, "replaces-call-id",
															   replaces->rp_call_id);
							}
							switch_event_add_header_string(xml_params, SWITCH_STACK_BOTTOM, "refer-from-channel-id",
														   switch_core_session_get_uuid(session));
							switch_event_add_header_string(xml_params, SWITCH_STACK_BOTTOM, "refer-for-channel-id",
														   br_a);

							if (switch_xml_locate("channels", NULL, NULL, NULL, &xml_root, &xml_channel, xml_params,
												  SWITCH_FALSE) == SWITCH_STATUS_SUCCESS) {
								...
								if (absolute_dial_string) {
									exten =
										switch_core_session_sprintf(session, "%s%s", dial_prefix, absolute_dial_string);
								} else if (xml_url) {
									exten = switch_core_session_sprintf(session, "%ssofia/%s/%s", dial_prefix,
																		use_profile, xml_url);
								}
						...
						if (zstr(exten)) {
							exten = switch_core_session_sprintf(session, "sofia/%s/sip:%s@%s%s%s", profile->name,
																refer_to->r_url->url_user, refer_to->r_url->url_host,
																port ? ":" : "", port ? port : "");
						}
						...
						if (refer_to->r_url->url_params || refer_to->r_url->url_headers) {
							if (refer_to->r_url->url_headers) {
								nightmare_xfer_helper->exten_with_params =
									switch_core_sprintf(npool, "{sip_invite_params=%s?%s}%s",
														refer_to->r_url->url_params ? refer_to->r_url->url_params : "",
														refer_to->r_url->url_headers, exten);
							} else {
								nightmare_xfer_helper->exten_with_params = switch_core_sprintf(
									npool, "{sip_invite_params=%s}%s", refer_to->r_url->url_params, exten);
							}
						} else {
						...
						nightmare_xfer_helper->reply_uuid =
							switch_core_strdup(npool, switch_core_session_get_uuid(session));
							...
							while (p && *p) {
								...
								if ((p = strchr(p, '='))) {
									*p++ = '\0';
									...
									switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
													  "Exporting replaces URL header [%s:%s]\n", h, v);
								}
							}
						}
						...
						if (rep_h) {
							switch_event_add_header_string(nightmare_xfer_helper->vars, SWITCH_STACK_BOTTOM,
														   SOFIA_REPLACES_HEADER, rep_h);
						} else {
							switch_event_add_header_string(nightmare_xfer_helper->vars, SWITCH_STACK_BOTTOM,
														   SOFIA_REPLACES_HEADER, rep);
						}
						...
						if (!zstr(full_ref_to)) {
							switch_event_add_header_string(nightmare_xfer_helper->vars, SWITCH_STACK_BOTTOM,
														   SOFIA_REFER_TO_VARIABLE, full_ref_to);
						}
						...
						switch_event_add_header_string(nightmare_xfer_helper->vars, SWITCH_STACK_BOTTOM,
													   "sip_h_X-FS-Refer-From", switch_core_session_get_uuid(session));
						switch_event_add_header_string(nightmare_xfer_helper->vars, SWITCH_STACK_BOTTOM,
													   "sip_h_X-FS-Refer-For", br_a);
	...
	if (exten) {
		...
		if (!zstr(br) && (b_session = switch_core_session_locate(br))) {
			...
			if (!zstr(full_ref_to)) { switch_channel_set_variable(b_channel, SOFIA_REFER_TO_VARIABLE, full_ref_to); }
...
static switch_status_t create_info_event(sip_t const *sip, nua_handle_t *nh, switch_event_t **revent)
{
	...
	if (sip->sip_contact) {
		if (sip->sip_contact->m_url->url_user) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "SIP-Contact-User",
										   sip->sip_contact->m_url->url_user);
		}

		if (sip->sip_contact->m_url->url_host) {
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "SIP-Contact-Host",
										   sip->sip_contact->m_url->url_host);
		}
	}
	...
	if (alert_info) {
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Alert-Info",
									   sip_header_as_string(nua_handle_home(nh), (void *)alert_info));
	}
...
switch_status_t sofia_proxy_sip_i_message(nua_t *nua, sofia_profile_t *profile, nua_handle_t *nh,
										  switch_core_session_t *session, sip_t const *sip, sofia_dispatch_event_t *de,
										  tagi_t tags[])
{
	switch_core_session_t *other_session = NULL;
	const char *session_id_header = sofia_glue_session_id_header(session, profile);
	...
	if (session && switch_core_session_get_partner(session, &other_session) == SWITCH_STATUS_SUCCESS) {
		if (switch_core_session_compare(session, other_session)) {
			...
			if (sip && sip->sip_payload && sip->sip_payload->pl_data) { pl = sip->sip_payload->pl_data; }
...
switch_status_t sofia_proxy_sip_i_info(nua_t *nua, sofia_profile_t *profile, nua_handle_t *nh,
									   switch_core_session_t *session, sip_t const *sip, sofia_dispatch_event_t *de,
									   tagi_t tags[])
{
	switch_core_session_t *other_session = NULL;
	...
	if (session && switch_core_session_get_partner(session, &other_session) == SWITCH_STATUS_SUCCESS) {
		if (switch_core_session_compare(session, other_session)) {
			...
			if (sip && sip->sip_content_type && sip->sip_content_type->c_type &&
				!strncasecmp(sip->sip_content_type->c_type, "application", 11) &&
				!strcasecmp(sip->sip_content_type->c_subtype, "media_control+xml")) {
...
void sofia_handle_sip_i_info(nua_t *nua, sofia_profile_t *profile, nua_handle_t *nh, switch_core_session_t *session,
							 sip_t const *sip, sofia_dispatch_event_t *de, tagi_t tags[])
{
					...
					if (session) {
						...
						if (create_info_event(sip, nh, &event) == SWITCH_STATUS_SUCCESS) {
							if (switch_core_session_queue_event(session, &event) == SWITCH_STATUS_SUCCESS) {
								switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
												  "queued freeswitch event for INFO\n");
								nua_respond(
									nh, SIP_200_OK, SIPTAG_CONTENT_TYPE_STR("freeswitch/session-event-response"),
	...
	if (session) {
		...
		if (!zstr(profile->proxy_info_content_types) && sip && sip->sip_content_type && sip->sip_content_type->c_type &&
			sip->sip_content_type->c_subtype &&
			(!strcasecmp(profile->proxy_info_content_types, "all") ||
			 strstr(profile->proxy_info_content_types, sip->sip_content_type->c_type))) {
			switch_core_session_t *other_session;
			...
			if (switch_core_session_get_partner(session, &other_session) == SWITCH_STATUS_SUCCESS) {
				...
				nua_info(other_tech_pvt->nh, SIPTAG_CONTENT_TYPE_STR(ct),
						 TAG_IF(!zstr(extra_headers), SIPTAG_HEADER_STR(extra_headers)),
		...
		if (sip && sip->sip_content_type && sip->sip_content_type->c_type &&
			!strcasecmp(sip->sip_content_type->c_type, "freeswitch/data")) {
			char *data = NULL;

			if (sip->sip_payload && sip->sip_payload->pl_data) { data = sip->sip_payload->pl_data; }
		...
		if (sip && sip->sip_content_type && sip->sip_content_type->c_type && sip->sip_content_type->c_subtype &&
			sip->sip_payload && sip->sip_payload->pl_data) {
			if (!strncasecmp(sip->sip_content_type->c_type, "application", 11) &&
				!strcasecmp(sip->sip_content_type->c_subtype, "media_control+xml")) {
				...
				if ((signal_ptr = switch_stristr("d=", sip->sip_payload->pl_data))) {
					...
					if (*signal_ptr && (*signal_ptr == '*' || *signal_ptr == '#' || *signal_ptr == 'A' ||
										*signal_ptr == 'B' || *signal_ptr == 'C' || *signal_ptr == 'D')) {
					...
					goto end;
					   ...
					   !strcasecmp(sip->sip_content_type->c_subtype, "dtmf-relay")) {
				...
				if ((signal_ptr = switch_stristr("Duration=", sip->sip_payload->pl_data))) {
					...
					if ((tmp = atoi(signal_ptr)) <= 0) { tmp = switch_core_default_dtmf_duration(0); }
					dtmf.duration = tmp * 8;
				}
			} else if (!strncasecmp(sip->sip_content_type->c_type, "application", 11) &&
					   !strcasecmp(sip->sip_content_type->c_subtype, "dtmf")) {
			...
			if (dtmf.digit) {
				if (tech_pvt->mparams.dtmf_type == DTMF_INFO ||
					sofia_test_pflag(tech_pvt->profile, PFLAG_LIBERAL_DTMF) ||
					...
					if (switch_channel_test_flag(channel, CF_PROXY_MODE)) {
						...
						if ((uuid = switch_channel_get_partner_uuid(channel)) &&
							(session_b = switch_core_session_locate(uuid))) {
		...
		if ((rec_header = sofia_glue_get_unknown_header(sip, "record"))) {
			if (zstr(profile->record_template)) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
								  "Record attempted but no template defined.\n");
				...
				if (!strcasecmp(rec_header, "on")) {
					...
					if (switch_true(switch_channel_get_variable(channel, "sip_disable_recording"))) {
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
										  "Record attempted but is disabled by sip_disable_recording variable.\n");
						nua_respond(nh, 488, "Recording disabled for this channel",
									NUTAG_WITH_THIS_MSG(de->data->e_msg),
									TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)), TAG_END());
					} else {

						tmp =
							switch_mprintf("%s%s%s", profile->record_path ? profile->record_path : "${recordings_dir}",
	...
	if (create_info_event(sip, nh, &event) == SWITCH_STATUS_SUCCESS) {
		...
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG1,
						  "dispatched freeswitch event for INFO\n");
	}
								 ...
								 tagi_t tags[])
{
	...
	if (session) {
		...
		//updated = (char *)switch_channel_get_private(channel, "received_sip_update");
		/*if (switch_channel_get_private(channel, "received_sip_update")) {
			if (switch_event_create(&event, SWITCH_EVENT_CUSTOM) == SWITCH_STATUS_SUCCESS) {
				char *uuid = switch_core_session_get_uuid(session);
				event->subclass_name = strdup("sofia::events");
				switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Event-Subclass", event->subclass_name);
				switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Action", "reinvite");
				switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "my_uuid", uuid);
				switch_event_fire(&event);
			}
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "received nua_i_reinvite .\n");
		}*/
	}
	if (session && profile && sip && sofia_test_pflag(profile, PFLAG_TRACK_CALLS)) {
		...
		switch_snprintf(via_space, sizeof(via_space), "SIP/2.0/UDP %s;rport=%d;branch=%s", network_ip, network_port,
						branch);
...
switch_status_t sofia_locate_user(char *user, switch_core_session_t *session, sip_t const *sip, switch_xml_t *x_user)
{
	...
	if (zstr(user)) { return SWITCH_STATUS_FALSE; }

	if (!(username = switch_core_session_strdup(session, user))) { return SWITCH_STATUS_FALSE; }

	if (!(domain = strchr(username, '@'))) { return SWITCH_STATUS_FALSE; }
...
void sofia_handle_sip_i_invite(switch_core_session_t *session, nua_t *nua, sofia_profile_t *profile, nua_handle_t *nh,
							   sofia_private_t *sofia_private, sip_t const *sip, sofia_dispatch_event_t *de,
							   tagi_t tags[])
{
	char key[128] = "";
	...
	const char *dialog_from_user = "", *dialog_from_host = "", *to_user = "", *to_host = "", *contact_user = "",
			   *contact_host = "";
	const char *user_agent = "", *call_id = "";
	url_t *from = NULL, *to = NULL, *contact = NULL;
	const char *to_tag = "";
	...
	if (sip && sip->sip_payload && sip->sip_payload->pl_data) { r_sdp = sip->sip_payload->pl_data; }
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(tech_pvt->session), SWITCH_LOG_INFO,
					  "%s receiving invite from %s:%d version: %s call-id: %s\n",
					  switch_channel_get_name(tech_pvt->channel), network_ip, network_port, switch_version_full_human(),
					  sip->sip_call_id ? switch_str_nil(sip->sip_call_id->i_id) : "");

	if (sip->sip_via && sip->sip_via->v_protocol && switch_stristr("sip/2.0/ws", sip->sip_via->v_protocol)) {
		is_nat = "websockets";
	}

	if (r_sdp) { switch_core_media_set_sdp_codec_string(session, r_sdp, SDP_OFFER); }

	if (sofia_test_pflag(profile, PFLAG_AGGRESSIVE_NAT_DETECTION) ||
		(sofia_test_pflag(profile, PFLAG_TLS_ALWAYS_NAT) && (is_tcp || is_tls)) ||
		(!is_tcp && !is_tls &&
		 (zstr(network_ip) || !switch_check_network_list_ip(network_ip, profile->local_network)) &&
	...
	if (!is_nat && profile->nat_acl_count) {
		...
		if (!zstr(contact_host)) {
			/* NAT mode double check logic and examples.
			   ...
			   Example 2: the contact_host is 192.168.1.100 and the network_ip is 192.0.2.100 which is a public internet
			   ip the remote endpoint is likely behind a remote nat traversing the public internet.
			   ...
			   Why would you want to do this?  Well if your FreeSWITCH is behind nat and you want to talk to endpoints
			   behind remote NAT over the public internet in addition to endpoints behind nat with you.  This simplifies
			   that process.
			...
			*/
			...
			for (x = 0; x < profile->nat_acl_count; x++) {
				...
				if ((contact_private_ip = switch_check_network_list_ip(contact_host, last_acl))) { break; }
			}
			if (contact_private_ip) {
				for (x = 0; x < profile->nat_acl_count; x++) {
					if ((network_private_ip = switch_check_network_list_ip(network_ip, profile->nat_acl[x]))) { break; }
				}
			}

			if (contact_private_ip && !network_private_ip) { is_nat = last_acl; }
		}
	}
	...
	if (profile->acl_count) {
		...
		for (x = 0; x < profile->acl_count; x++) {
			...
			if ((ok = switch_check_network_list_ip_port_token(network_ip, acl_port, last_acl, &token))) {
				...
				if (profile->acl_pass_context[x]) { acl_context = profile->acl_pass_context[x]; }
				if (!token && profile->acl_inbound_x_token_header) {
					...
					if (!zstr(x_auth_token)) { token = x_auth_token; }
				}
		...
		if (ok) {
			if (token) { switch_set_string(acl_token, token); }
			if (sofia_test_pflag(profile, PFLAG_AUTH_CALLS)) {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
								  "IP %s Approved by acl \"%s[%s]\". Access Granted.\n", network_ip,
								  switch_str_nil(last_acl), acl_token);
			...
			if (network_ip_is_proxy) {
				...
				int x_auth_port_i = sofia_test_pflag(profile, PFLAG_USE_PORT_FOR_ACL_CHECK)
										? zstr(x_auth_port) ? 0 : atoi(x_auth_port)
										: 0;
				...
				if (!ok && (x_auth_ip = sofia_glue_get_unknown_header(sip, "X-AUTH-IP")) && !zstr(x_auth_ip)) {
					for (x = 0; x < profile->acl_count; x++) {
						last_acl = profile->acl[x];
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
										  "verifying acl \"%s\" from proxy for ip/port %s:%i.\n",
										  switch_str_nil(last_acl), x_auth_ip, x_auth_port_i);
						if ((ok =
								 switch_check_network_list_ip_port_token(x_auth_ip, x_auth_port_i, last_acl, &token))) {
			...
			if (ok) {
				...
				if (!sofia_test_pflag(profile, PFLAG_AUTH_CALLS)) {
					...
					if (!acl_context) {
						...
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
										  "IP %s Rejected by acl \"%s\". Falling back to Digest auth.\n", x_auth_ip,
	...
	if (!is_auth && sofia_test_pflag(profile, PFLAG_AUTH_CALLS) &&
		sofia_test_pflag(profile, PFLAG_AUTH_CALLS_ACL_ONLY)) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
						  "IP/Port %s %i Rejected by acls and auth-calls-acl-only flag is set, rejecting call\n",
						  network_ip, network_port);
		nua_respond(nh, SIP_403_FORBIDDEN, TAG_IF(!zstr(session_id_header), SIPTAG_HEADER_STR(session_id_header)),
					TAG_END());
		goto fail;
	}
	...
	if (!is_auth && sofia_test_pflag(profile, PFLAG_AUTH_CALLS) && sofia_test_pflag(profile, PFLAG_BLIND_AUTH)) {
		...
		if (!strcmp(network_ip, profile->sipip) && network_port == profile->sip_port) { calling_myself++; }

		if (sip->sip_from) {
			user = switch_core_session_sprintf(session, "%s@%s", sip->sip_from->a_url->url_user,
											   sip->sip_from->a_url->url_host);
		...
		if (!sofia_test_pflag(profile, PFLAG_BLIND_AUTH_ENFORCE_RESULT) || blind_result == SWITCH_STATUS_SUCCESS) {
			...
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
							  "blind auth enforce 403 enabled and couldn't find user %s, rejecting call\n", user);
			nua_respond(nh, SIP_403_FORBIDDEN, TAG_END());
			goto fail;
		}
	}

	if (sip->sip_from) { tech_pvt->from_user = switch_core_session_strdup(session, sip->sip_from->a_url->url_user); }
	...
	if (!is_auth &&
		...
		if (!strcmp(network_ip, profile->sipip) && network_port == profile->sip_port) {
			...
			if (sofia_reg_handle_register(nua, profile, nh, sip, de, REG_INVITE, key, sizeof(key), &v_event, NULL, NULL,
										  &x_user)) {
				if (v_event) { switch_event_destroy(&v_event); }
				if (x_user) { switch_xml_free(x_user); }

				if (sip->sip_authorization || sip->sip_proxy_authorization) { goto fail; }
	...
	if (sip->sip_via) {
		...
		tech_pvt->record_route = switch_core_session_sprintf(
			session, "sip:%s%s%s:%d;transport=%s", ipv6 ? "[" : "", tech_pvt->mparams.remote_ip, ipv6 ? "]" : "",
		...
		switch_channel_set_variable(channel, "sip_via_protocol",
									sofia_glue_transport2str(sofia_glue_via2transport(sip->sip_via)));
	}

	if (*key != '\0') { tech_pvt->key = switch_core_session_strdup(session, key); }

	if (is_auth) {
		switch_channel_set_variable(channel, "sip_authorized", "true");

		if (!zstr(sip_acl_authed_by)) { switch_channel_set_variable(channel, "sip_acl_authed_by", sip_acl_authed_by); }

		if (!zstr(sip_acl_token)) { switch_channel_set_variable(channel, "sip_acl_token", sip_acl_token); }
	}

	if (calling_myself) { switch_channel_set_variable(channel, "sip_looped_call", "true"); }

	tech_pvt->caller_profile = switch_caller_profile_new(switch_core_session_get_pool(session), NULL, NULL, NULL, NULL,
														 NULL, NULL, NULL, NULL, MODNAME, NULL, NULL);
	switch_channel_set_caller_profile(channel, tech_pvt->caller_profile);

	if (x_user) {
		const char *ruser = NULL, *rdomain = NULL, *user = switch_xml_attr(x_user, "id"),
				   *domain = switch_xml_attr(x_user, "domain-name");
	...
	if (sip->sip_from) {
		...
		if (sip->sip_from->a_url->url_params) {
			aniii = switch_find_parameter(sip->sip_from->a_url->url_params, "isup-oli",
		...
		if (sip->sip_from->a_params) { set_variable_sip_param(channel, "from", sip->sip_from->a_params); }
	...
	if ((rpid = sip_remote_party_id(sip))) {
		...
		if (!zstr(rpid->rpid_display)) { displayname = rpid->rpid_display; }
			...
			if (!zstr(full_paid_header)) {
				if (profile->paid_type == PAID_DEFAULT || profile->paid_type == PAID_USER) {
					...
					switch_channel_set_variable(channel, "sip_P-Asserted-Identity",
												switch_core_session_sprintf(session, "%s@%s",
																			passerted->paid_url->url_user,
																			passerted->paid_url->url_host));
				} else if (profile->paid_type == PAID_VERBATIM) {
					switch_channel_set_variable(channel, "sip_P-Asserted-Identity", full_paid_header);
				}
			}
		}
		if (!zstr(passerted->paid_display)) { displayname = passerted->paid_display; }
	...
	if ((ppreferred = sip_p_preferred_identity(sip))) {
		...
		if (!zstr(ppreferred->ppid_display)) { displayname = ppreferred->ppid_display; }
		switch_channel_set_variable(channel, "sip_cid_type", "pid");
		tech_pvt->cid_type = CID_TYPE_PID;
	}
				...
				if ((argc1 = switch_separate_string(req_user, ';', argv1, (sizeof(argv1) / sizeof(argv1[0]))))) {
					for (x1 = 0; x1 < argc1; x1++) {
						if (x1 == 0) {
							...
							char *argv2[2] = {0};
							if (switch_separate_string(argv1[x1], '=', argv2, (sizeof(argv2) / sizeof(argv2[0]))) ==
								2) {
								char *var_name = NULL;
	...
	if (sip->sip_request->rq_url->url_params &&
		(sofia_glue_find_parameter(sip->sip_request->rq_url->url_params, "intercom=true"))) {
		switch_channel_set_variable(channel, "sip_auto_answer_detected", "true");
	}

	if (!destination_number && sip->sip_to) { destination_number = sip->sip_to->a_url->url_user; }

	/* The human network, OH THE HUMANITY!!! lets send invites with no number! */
	if (!destination_number && sip->sip_from) { destination_number = sip->sip_from->a_url->url_user; }
	...
	if (sip->sip_to) {
		...
		if (switch_channel_get_variable(channel, "sip_to_uri")) {
			...
			if (sip->sip_to->a_params) { set_variable_sip_param(channel, "to", sip->sip_to->a_params); }
			...
			ipv6 = strchr(host, ':');
			tech_pvt->to_uri =
				switch_core_session_sprintf(session, "sip:%s@%s%s%s:%d;transport=%s", user, ipv6 ? "[" : "", host,
											ipv6 ? "]" : "", port, sofia_glue_transport2str(transport));

			if (sofia_glue_check_nat(profile, tech_pvt->mparams.remote_ip)) { check_nat = 1; }
			url = sofia_glue_get_profile_url(profile, tech_pvt->mparams.remote_ip, transport);

			if (!url) {
				if (check_nat) {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
									  "Nat detected but no external address configured.\n");
				}
				url = profile->url;
			}

			if (!url) { switch_channel_hangup(tech_pvt->channel, SWITCH_CAUSE_DESTINATION_OUT_OF_ORDER); }

			tmp = sofia_overcome_sip_uri_weakness(session, url, transport, SWITCH_TRUE, NULL, NULL);

			if ((at = strchr(tmp, '@'))) { url = switch_core_session_sprintf(session, "sip:%s%s", user, at); }
			...
			if (url) {
				...
				tech_pvt->reply_contact = switch_core_session_sprintf(
					session, "%s%s%s%s%s", brackets ? "" : "<", url,
					proto ? "" : ";transport=", proto ? "" : sofia_glue_transport2str(transport), brackets ? "" : ">");
			} else {
	...
	if (sip->sip_referred_by) {
		...
		if (!zstr(referred_by_user)) {
			if (*referred_by_user == '+') {
				switch_channel_set_variable(channel, "sip_referred_by_user_stripped",
											(const char *)(referred_by_user + 1));
			} else {
	...
	if (sofia_test_flag(tech_pvt, TFLAG_INB_NOMEDIA)) { switch_channel_set_flag(channel, CF_PROXY_MODE); }

	if (profile->media_options & MEDIA_OPT_BYPASS_AFTER_HOLD) {
		switch_channel_set_flag(channel, CF_BYPASS_MEDIA_AFTER_HOLD);
	}

	if (sofia_test_flag(tech_pvt, TFLAG_PROXY_MEDIA)) { switch_channel_set_flag(channel, CF_PROXY_MEDIA); }
	...
	if (sip->sip_via) {
		if (sip->sip_via->v_host) { switch_channel_set_variable(channel, "sip_via_host", sip->sip_via->v_host); }
		if (sip->sip_via->v_port) { switch_channel_set_variable(channel, "sip_via_port", sip->sip_via->v_port); }
		if (sip->sip_via->v_rport) { switch_channel_set_variable(channel, "sip_via_rport", sip->sip_via->v_rport); }
	}
	...
	if (sip->sip_multipart) {
		...
		for (mp = sip->sip_multipart; mp; mp = mp->mp_next) {
			if (mp->mp_payload && mp->mp_payload->pl_data && mp->mp_content_type && mp->mp_content_type->c_type) {
				char *val =
					switch_core_session_sprintf(session, "%s:%s", mp->mp_content_type->c_type, mp->mp_payload->pl_data);
	...
	if (!context) { context = switch_channel_get_variable(channel, "user_context"); }
	...
	if (!(dialplan = switch_channel_get_variable(channel, "inbound_dialplan"))) { dialplan = profile->dialplan; }
	...
	if ((call_info = sip_call_info(sip))) {
		...
		while (call_info) {
			call_info_str = sip_header_as_string(nua_handle_get_home(nh), (void *)call_info);
			switch_channel_add_variable_var_check(channel, "sip_call_info", call_info_str, SWITCH_FALSE,
												  SWITCH_STACK_PUSH);
			call_info = call_info->ci_next;
		}
		...
		if (sip->sip_from && sip->sip_from->a_url->url_user && sip->sip_from->a_url->url_host && sip->sip_to &&
			...
			if (!zstr(buf)) {
				...
				switch_channel_set_variable(channel, "presence_call_info", buf);
				call_info_str =
					switch_core_session_sprintf(session, "<sip:%s>;%s", sip->sip_from->a_url->url_host, buf);
			}
		}
	}
	...
	if (sip->sip_request->rq_url->url_params) {
		gw_param_name =
			switch_find_parameter(sip->sip_request->rq_url->url_params, "gw", switch_core_session_get_pool(session));
	}
	...
	if (gw_name || gw_param_name) {
		...
		if (!gateway && gw_param_name) {
			if ((gateway = sofia_reg_find_gateway(gw_param_name))) { extension = gateway->real_extension; }
		}
		...
		if (gateway) {
			...
			if (!zstr(extension)) {
				if (!strcasecmp(extension, "auto_to_user") && sip->sip_to) {
					...
					if (gw_name && sip->sip_to) { destination_number = sip->sip_to->a_url->url_user; }
				} else {
			...
			if (gateway->ib_vars) {
				...
				for (hp = gateway->ib_vars->headers; hp; hp = hp->next) {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
									  "%s setting variable [%s]=[%s]\n", switch_channel_get_name(channel), hp->name,
									  hp->value);
					switch_channel_set_variable(channel, hp->name, hp->value);
				}
			}
	...
	if (bnh) {
		...
		if ((b_private = nua_handle_magic(bnh))) {
			...
			if ((b_session = switch_core_session_locate(b_private->uuid))) {
				switch_channel_t *b_channel = switch_core_session_get_channel(b_session);
				sofia_handle_sip_i_invite_replaces(session, channel, b_channel, b_private->uuid, tech_pvt, call_info,
												   profile, is_nat, sip);
				switch_core_session_rwunlock(b_session);
			}
		}
		...
		if ((b_session = switch_core_session_locate((char *)sip->sip_replaces->rp_call_id))) {
			switch_channel_t *b_channel = switch_core_session_get_channel(b_session);
			sofia_handle_sip_i_invite_replaces(session, channel, b_channel, (char *)sip->sip_replaces->rp_call_id,
											   tech_pvt, call_info, profile, is_nat, sip);
			switch_core_session_rwunlock(b_session);
		}
	}
	...
	if (tech_pvt->caller_profile) {
		...
		if ((privacy = sip_privacy(sip))) {
			...
			if (!zstr(full_priv_header)) { switch_channel_set_variable(channel, "sip_Privacy", full_priv_header); }
		...
		if (sip->sip_identity) {
			...
			for (id = sip->sip_identity; id; id = id->id_next) {
				if (!zstr(id->id_value)) {
					switch_channel_add_variable_var_check(channel, "sip_identity", id->id_value, SWITCH_FALSE,
														  SWITCH_STACK_PUSH);
				}
			}
		}
		...
		if (sip->sip_date && sip->sip_date->d_time > 0) {
			...
			switch_channel_set_variable_printf(channel, "sip_date_epoch_time", "%ld",
											   sip->sip_date->d_time - 2208988800);
		}
		...
		for (un = sip->sip_unknown; un; un = un->un_next) {
			if (!strncasecmp(un->un_name, "Accept-Language", 15)) {
				...
				switch_channel_set_variable(channel, "sip_user_location", un->un_value);
					   ...
					   !strcasecmp(un->un_name, "User-to-User") || !strncasecmp(un->un_name, "On", 2) ||
					   !strncasecmp(un->un_name, "K-", 2)) {
				if (!zstr(un->un_value)) {
					char new_name[512] = "";
					int reps = 0;
					for (;;) {
						char postfix[25] = "";
						if (reps > 0) { switch_snprintf(postfix, sizeof(postfix), "-%d", reps); }
						reps++;
						switch_snprintf(new_name, sizeof(new_name), "%s%s%s", SOFIA_SIP_HEADER_PREFIX, un->un_name,
										postfix);

						if (switch_channel_get_variable(channel, new_name)) { continue; }
	...
	if (sofia_test_pflag(profile, PFLAG_PARSE_ALL_INVITE_HEADERS)) { sofia_parse_all_invite_headers(sip, session, nh); }
	...
	if (profile->pres_type) {
		...
		sql = switch_mprintf(
			...
			"'')",
			call_id, tech_pvt->sofia_private->uuid, to_user, to_host, to_tag, dialog_from_user, dialog_from_host,
			from_tag, contact_user, contact_host, "confirmed", "inbound", user_agent, profile->name,
			mod_sofia_globals.hostname, switch_str_nil(full_contact), switch_str_nil(presence_id),
			switch_str_nil(presence_data), switch_str_nil(p), now);
...
void sofia_handle_sip_i_invite_replaces(switch_core_session_t *session, switch_channel_t *channel,
										switch_channel_t *b_channel, char *uuid, private_object_t *tech_pvt,
										sip_call_info_t *call_info, sofia_profile_t *profile, char *is_nat,
										sip_t const *sip)
{
	...
	if (orig_cp) {
		tech_pvt->caller_profile->callee_id_name =
			switch_core_strdup(tech_pvt->caller_profile->pool, orig_cp->callee_id_name);
		tech_pvt->caller_profile->callee_id_number =
			switch_core_strdup(tech_pvt->caller_profile->pool, orig_cp->callee_id_number);

		if (!call_info) {
			tech_pvt->caller_profile->caller_id_name =
				switch_core_strdup(tech_pvt->caller_profile->pool, orig_cp->caller_id_name);
			tech_pvt->caller_profile->caller_id_number =
				switch_core_strdup(tech_pvt->caller_profile->pool, orig_cp->caller_id_number);
		}
	...
	if (bridge_uuid) {
		...
		if ((bridge_session = switch_core_session_locate(bridge_uuid))) {
			...
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
							  "could not locate partner_uuid %s, resetting\n", bridge_uuid);
			bridge_uuid = NULL;
		}
	}
	...
	if (call_info) {
		...
		if (!zstr(bridge_uuid) && switch_channel_test_flag(b_channel, CF_LEG_HOLDING)) {
			...
			if (b_call_id) {
				char *sql =
					switch_mprintf("update sip_dialogs set call_info_state='idle' where call_id='%q'", b_call_id);
				if (mod_sofia_globals.debug_sla > 1) {
			...
			tech_pvt->caller_profile->destination_number =
				switch_core_sprintf(tech_pvt->caller_profile->pool, "answer,intercept:%s", bridge_uuid);

			if (sofia_test_pflag(profile, PFLAG_FIRE_TRANFER_EVENTS) && sip && sip->sip_call_id &&
			...
			if (!zstr(bcp->callee_id_name)) {
				tech_pvt->caller_profile->callee_id_name =
					switch_core_strdup(tech_pvt->caller_profile->pool, bcp->callee_id_name);
			}

			if (!zstr(bcp->callee_id_number)) {
				tech_pvt->caller_profile->callee_id_number =
					switch_core_strdup(tech_pvt->caller_profile->pool, bcp->callee_id_number);
			}

			if (!zstr(bcp->caller_id_name)) {
				tech_pvt->caller_profile->caller_id_name =
					switch_core_strdup(tech_pvt->caller_profile->pool, bcp->caller_id_name);
			}

			if (!zstr(bcp->caller_id_number)) {
				tech_pvt->caller_profile->caller_id_number =
					switch_core_strdup(tech_pvt->caller_profile->pool, bcp->caller_id_number);
			}

			if (bcp->originatee_caller_profile) {
				switch_caller_profile_t *cp;

				cp = switch_caller_profile_dup(tech_pvt->caller_profile->pool, bcp->originatee_caller_profile);

				switch_channel_set_originatee_caller_profile(tech_pvt->channel, cp);
			}

			tech_pvt->caller_profile->destination_number =
				switch_core_sprintf(tech_pvt->caller_profile->pool, "answer,sofia_sla:%s", uuid);
		}
	} else {
		...
		if (switch_channel_var_true(channel, "sip_replaces_a-leg")) {
			...
			if (sip) {
				tech_pvt->caller_profile->destination_number = switch_core_sprintf(
					tech_pvt->caller_profile->pool, "answer,intercept:%s", sip->sip_replaces->rp_call_id);
				...
				if (sofia_test_pflag(profile, PFLAG_FIRE_TRANFER_EVENTS) && sip->sip_call_id &&
					switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, MY_EVENT_INTERCEPTED) ==
						SWITCH_STATUS_SUCCESS) {
					switch_channel_event_set_data(b_channel, event);
					switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "intercepted_by",
												   sip->sip_call_id->i_id);
					switch_event_fire(&event);
				}
			}
			...
			if (!zstr(bridge_uuid)) {
				if (sip && sip->sip_replaces && sip->sip_replaces->rp_params && sip->sip_replaces->rp_call_id &&
					switch_channel_test_flag(b_channel, CF_BRIDGED) &&
					switch_true(switch_find_parameter(*(sip->sip_replaces->rp_params), "early-only",
													  switch_core_session_get_pool(session)))) {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
									  "call %s intercept rejected\n", bridge_uuid);
					tech_pvt->caller_profile->destination_number =
						switch_core_sprintf(tech_pvt->caller_profile->pool, "hangup:CALL_REJECTED");
				} else {
					switch_channel_mark_hold(b_channel, SWITCH_FALSE);
					tech_pvt->caller_profile->destination_number =
						switch_core_sprintf(tech_pvt->caller_profile->pool, "answer,intercept:%s", bridge_uuid);
				...
				if (b_data && b_app) {
					tech_pvt->caller_profile->destination_number =
						switch_core_sprintf(tech_pvt->caller_profile->pool, "answer,%s:%s", b_app, b_data);
				} else if (b_app) {
					tech_pvt->caller_profile->destination_number =
						switch_core_sprintf(tech_pvt->caller_profile->pool, "answer,%s", b_app);
				}
				if (sofia_test_pflag(profile, PFLAG_FIRE_TRANFER_EVENTS) && sip && sip->sip_call_id &&
					switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, MY_EVENT_REPLACED) ==
						SWITCH_STATUS_SUCCESS) {
					switch_channel_event_set_data(b_channel, event);
					switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "att_xfer_replaced_by",
												   sip->sip_call_id->i_id);
...
void sofia_handle_sip_i_options(int status, char const *phrase, nua_t *nua, sofia_profile_t *profile, nua_handle_t *nh,
								sofia_private_t *sofia_private, sip_t const *sip, sofia_dispatch_event_t *de,
								tagi_t tags[])
{
	uint32_t sess_count = switch_core_session_count();
...
static void set_variable_sip_param(switch_channel_t *channel, char *header_type, sip_param_t const *params)
{
	...
	*sh = '\0';
	...
	for (cp = header_type; (*cp && (sh < (sh_end - 1))); cp++, sh++) { *sh = *cp; }
	*sh++ = '_';
	*sh = '\0';
	...
	while (params && params[0]) {
		...
		for (cp = (char *)(*params); ((*cp != '=') && *cp && (sh < sh_end)); cp++, sh++) { *sh = *cp; }
