...
static void add_des(char *buf, uint32_t buflen)
{
	switch_snprintf(buf + strlen(buf), buflen - strlen(buf), "a=curr:qos local none\r\n");
	switch_snprintf(buf + strlen(buf), buflen - strlen(buf), "a=curr:qos remote none\r\n");
	switch_snprintf(buf + strlen(buf), buflen - strlen(buf), "a=des:qos mandatory local sendrecv\r\n");
	switch_snprintf(buf + strlen(buf), buflen - strlen(buf), "a=des:qos optional remote sendrecv\r\n");
...
SWITCH_DECLARE(void) switch_core_media_gen_local_sdp(switch_core_session_t *session, switch_sdp_type_t sdp_type, const char *ip, switch_port_t port, const char *sr, int force)
{
	...
	if (!smh->payload_space) {
		...
		ptStartCode = switch_channel_get_variable(session->channel, "ptStartCode");
		if (switch_true(ptStartCode)) {
                        smh->payload_space = atoi(ptStartCode);
                }
	...
	if ((mbw = switch_channel_get_variable(smh->session->channel, "rtp_max_bandwidth"))) {
		bw = switch_parse_bandwidth_string(mbw);
		switch_snprintf(buf, SDPBUFLEN,
			...
			"t=0 0\r\n"
			"%s",
			username, smh->owner_id, smh->session_id, family, ip, username, bw, family, ip, srbuf);
	}
	else {
		switch_snprintf(buf, SDPBUFLEN,
			"v=0\r\n"
...
switch_media_flow_t media_flow_get_mode_from_string(const char *s)
{
	if (!strcmp("inactive", s)) {
		return SWITCH_MEDIA_FLOW_INACTIVE;
	} else if (!strcmp("sendrecv", s)) {
		return SWITCH_MEDIA_FLOW_SENDRECV;
	} else if (!strcmp("sendonly", s)) {
		return SWITCH_MEDIA_FLOW_SENDONLY;
	} else if (!strcmp("recvonly", s)) {
		return SWITCH_MEDIA_FLOW_RECVONLY;
	} else  {
	...
	case SWITCH_MESSAGE_INDICATE_MEDIA_RENEG:
		{
			...
			if (msg->string_arg) {
				...
				switch_core_media_gen_local_sdp(session, SDP_OFFER, NULL, 0, sr, 1);
			}
