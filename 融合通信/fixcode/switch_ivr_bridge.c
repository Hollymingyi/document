	...
	while (switch_channel_up_nosig(channel) && switch_channel_up_nosig(b_channel) && vh->up == 1) {
		if (switch_channel_media_up(channel)) {
			if (switch_core_session_transcoding(vh->session_a, vh->session_b, SWITCH_MEDIA_TYPE_VIDEO)) {
				...
				if (!switch_channel_test_flag(channel, CF_PROXY_MEDIA) && switch_true(switch_channel_get_variable(b_channel, "rtp_force_transcode"))) {
					...
					if (session_a_codec_implementation.iananame && (!switch_channel_test_flag(channel, CF_VIDEO_DECODED_READ) 
						...
						if (!strcmp(session_a_codec_implementation.iananame, "H264")) {
							switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Transcoding %s\n", session_a_codec_implementation.iananame);
