...
static switch_status_t open_encoder(h264_codec_context_t *context, uint32_t width, uint32_t height)
{
	...
	context->encoder_ctx->gop_size = 30;
	context->encoder_ctx->keyint_min = 30;
	...
	context->encoder_ctx->framerate = (AVRational){30, 1};
...
static switch_status_t switch_h264_encode(switch_codec_t *codec, switch_frame_t *frame)
{
	...
	//switch_time_t ct = switch_time_now();
	//switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "Before Generate/Send AV KEYFRAME, need_key_frame:%d, context->last_keyframe_request:%lld,avcodec_globals.key_frame_min_freq:%d,now : %lld \n",context->need_key_frame, (long long)context->last_keyframe_request, avcodec_globals.key_frame_min_freq,(long long)switch_time_now());	
	...
	if (avframe->key_frame == 0 && (context->last_keyframe_request + 1000000) < switch_time_now()){
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "Interval to make AV KEYFRAME\n");
        avframe->pict_type = AV_PICTURE_TYPE_I;
        avframe->key_frame = 1;
        context->last_keyframe_request = switch_time_now();
