...
#define KEY_MAX_INTERVAL_ENABLE 1
#define KEY_MAX_INTERVAL 2000000 //2s
...
#if KEY_MAX_INTERVAL_ENABLE
	switch_time_t key_time; // switch_time_now()
	switch_time_t key_max_interval; // us
	int32_t enc_cnt;
#endif
...
#if KEY_MAX_INTERVAL_ENABLE
	context->key_max_interval = KEY_MAX_INTERVAL; //us
	context->key_time = switch_time_now() + context->key_max_interval;
	context->enc_cnt = 0;
#endif
...
static switch_status_t switch_h264_encode(switch_codec_t *codec, switch_frame_t *frame)
{
	...
	if (context->key_time) {
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "H264 ENC %d FRAME at %d ms\n", context->enc_cnt++, ct /1000);
		if (ct >= context->key_time) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "REQUEST H264 KEYFRAME GENERATED at %d ms\n", ct /1000);
			context->encoder->ForceIntraFrame(1);
			context->key_time = ct + context->key_max_interval;
		}
	}
#endif
