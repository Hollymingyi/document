...
#define H264_TRANSCODE_ENABLE 1
...
#if H264_TRANSCODE_ENABLE
				if (!switch_channel_test_flag(channel, CF_PROXY_MEDIA) && switch_true(switch_channel_get_variable(channel, "rtp_video_force_codec"))) {
					...
					if (session_a_codec_implementation.iananame && (!switch_channel_test_flag(channel, CF_VIDEO_DECODED_READ) 
						|| !switch_channel_test_flag(b_channel, CF_VIDEO_DECODED_READ))) {
						if (!strcmp(session_a_codec_implementation.iananame, "H264")) {
							switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "force codec %s\n", session_a_codec_implementation.iananame);
							switch_channel_set_flag_recursive(channel, CF_VIDEO_DECODED_READ);
							switch_channel_set_flag_recursive(b_channel, CF_VIDEO_DECODED_READ);
							pass_val = 1;
						}
					}
				}
